...
- [ ] Generate `.seb` config with correct `start_url`, `config_key`, and allowed applications
- [ ] Store Browser Exam Key in `lib/constants/business-rules.ts` (server-side only)
- [ ] Add `X-SafeExamBrowser-RequestHash` validation middleware to exam routes
- [ ] Add `sebRequired` boolean to `InternalExamClassSchedule`
- [ ] Client-side SEB detection: `navigator.userAgent` contains "SafeExamBrowser" or
...
- [ ] `bun run db:push:supabase` — **PENDING** (network connectivity — Supabase reachability issue; Neon is primary)
- [ ] Enable Realtime on `internal_exam_sessions` in Supabase Console — **PENDING** (requires Supabase access)
