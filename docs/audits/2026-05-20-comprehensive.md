...
| SEC-13 | No root middleware for portal role enforcement | ✅ | [proxy.ts:37](../../proxy.ts#L37) — `proxy()` (renamed from `middleware`) gates `/staff`, `/instructor`, `/student`, `/examiner`, `/applicant` and `/api/*` siblings via `ROUTE_ROLE_MAP`. Build registers it as `ƒ Proxy (Middleware)`. |
