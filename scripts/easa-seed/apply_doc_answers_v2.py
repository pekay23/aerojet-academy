...
# Parse the "WITH ANSWERS" doc-text file more carefully
# This file has questions numbered 1-52 with * marked answers
def parse_with_answers_v2(path):
...
for fname in sorted(os.listdir('doc-text/M2')):
    if fname.endswith('.txt'):
        ...
        entries = parse_with_answers_v2(path)
...
def find_doc_answer(csv_text):
    ...
    for q, a in all_doc_answers:
        ...
        if len(ct) > 50 and len(ql) > 50 and ct[:50] == ql[:50]:
            return a
        # Try containment
