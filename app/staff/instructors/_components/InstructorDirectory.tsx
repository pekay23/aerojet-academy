    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-4">
        ...
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-800/50 dark:bg-amber-900/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-amber-800 dark:text-amber-200">{expiringQuals}</p>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-xl border border-slate-200 bg-white py-3 pl-10 pr-4 text-sm focus:border-aerojet-blue focus:outline-none focus:ring-1 focus:ring-aerojet-blue dark:border-slate-700 dark:bg-slate-900"
        />
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {instructors.map(inst => {
            ...
            return (
              <div key={inst.id} className="rounded-xl border border-slate-200 bg-white transition-shadow hover:shadow-md dark:border-slate-800 dark:bg-slate-900">
                {/* Header Row */}
                <button
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="flex h-10 w-10 items-center justify-center rounded-full bg-aerojet-blue/10 text-aerojet-blue font-bold dark:bg-aerojet-sky/10 dark:text-aerojet-sky">
                      {inst.user.profile?.firstName?.[0]}{inst.user.profile?.lastName?.[0]}
                    </div>
                    <div>
                      <h3 className="font-bold text-slate-900 dark:text-white">{fullName}</h3>
                      <p className="text-xs text-slate-500">{inst.employeeId} • {inst.user.email}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    {/* Recency Badge */}
                    <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-bold ${
                      ...
                      {inst.recencyCompliant ? <CheckCircle2 className="h-3 w-3" /> : <XCircle className="h-3 w-3" />}
                ...
                {expanded && (
                  <div className="border-t border-slate-100 px-5 pb-5 dark:border-slate-800">
                    <div className="mt-4 grid gap-6 lg:grid-cols-2">
                      {/* Profile Info */}
                      <div>
                        <h4 className="mb-3 text-xs font-bold uppercase tracking-widest text-slate-400">Profile</h4>
                        <div className="space-y-2 text-sm">
                          <div className="flex justify-between"><span className="text-slate-500">Department</span><span className="font-medium text-slate-900 dark:text-white">{inst.department || '—'}</span></div>
                          <div className="flex justify-between"><span className="text-slate-500">Specialization</span><span className="font-medium text-slate-900 dark:text-white">{inst.specialization || '—'}</span></div>
                          <div className="flex justify-between"><span className="text-slate-500">Phone</span><span className="font-medium text-slate-900 dark:text-white">{inst.user.profile?.phone || '—'}</span></div>
                          <div className="flex justify-between"><span className="text-slate-500">Modules Qualified</span><span className="font-medium text-slate-900 dark:text-white">{inst.modulesQualified.length || 0}</span></div>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="mb-3 rounded-lg bg-slate-50 p-3 dark:bg-slate-800">
                          <div className="mb-1 flex justify-between text-sm">
                            <span className="text-slate-600 dark:text-slate-400">Update Training Hours</span>
                            <span className="font-bold text-slate-900 dark:text-white">{inst.recencyHours}h / 35h</span>
                          </div>
                          ...
                          <div className="space-y-1">
                            {inst.instructorRecency.slice(0, 5).map(r => (
                              <div key={r.id} className="flex items-center justify-between text-xs">
                                <span className="truncate text-slate-600 dark:text-slate-400">{r.description}</span>
                                <span className="ml-2 shrink-0 font-mono font-bold text-slate-900 dark:text-white">{r.hours}h</span>
                              </div>
                            ))}
                          </div>
                        )}
                    ...
                    {inst.instructorQualifications.length > 0 && (
                      <div className="mt-6">
                        <h4 className="mb-3 text-xs font-bold uppercase tracking-widest text-slate-400">Qualifications</h4>
                          ...
                          <table className="w-full text-left text-xs">
                            ...
                            <tbody className="divide-y dark:divide-slate-700">
                              {inst.instructorQualifications.map(q => {
                                ...
                                const expiringSoon = q.expiryDate && !expired && new Date(q.expiryDate) <= new Date(Date.now() + 90 * 24 * 60 * 60 * 1000)
                                return (
                                  <tr key={q.id}>
                                    <td className="p-3 font-medium text-slate-900 dark:text-white">{q.qualificationType}</td>
                                    <td className="p-3 text-slate-600 dark:text-slate-400">{q.issuedBy}</td>
                                    <td className="p-3 text-slate-600 dark:text-slate-400">{new Date(q.issueDate).toLocaleDateString()}</td>
                                    <td className="p-3 text-slate-600 dark:text-slate-400">{q.expiryDate ? new Date(q.expiryDate).toLocaleDateString() : 'N/A'}</td>
                                    <td className="p-3">
