...
export default function ExamBankManager() {
  ...
  const [rulesForm, setRulesForm] = useState({
    passMarkPct: ACADEMIC_RULES.EASA_PASS_MARK as number,
    timePerQuestionSecs: ACADEMIC_RULES.TIME_PER_QUESTION_SECS as number,
    retakeWaitDays: 90,
    maxRetakes: 3,
  ...
  const retireBank = async (bankId: string) => {
    if (
      !confirm(
        'Retire this bank? It will no longer be available for exams but existing sessions will be preserved.'
      )
    )
      return
    try {
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
            <BookOpen className="text-aerojet-blue h-5 w-5" />
        ...
        <button
          ...
          className="bg-aerojet-blue flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold text-white shadow-sm transition-all hover:bg-blue-700"
        >
          ...
          <div className="space-y-3">
            {banks.map((bank) => {
              ...
              const fillPct = Math.min(
                100,
                Math.round((bank.poolHealth.questionCount / bank.poolHealth.requiredMinimum) * 100)
              )
                ...
                >
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={`flex h-10 w-10 items-center justify-center rounded-lg ${health.bg}`}
                      >
                    ...
                    <div className="flex items-center gap-3">
                      <span
                        className={`rounded-full px-2.5 py-1 text-xs font-bold ${health.bg} ${health.color}`}
                      >
                        {health.label} ({bank.poolHealth.questionCount}/
                        {bank.poolHealth.requiredMinimum})
                      </span>
                      {(() => {
                        const rs =
                          REVIEW_STATE_CONFIG[bank.reviewState] || REVIEW_STATE_CONFIG.DRAFT
                        return (
                          <span
                            className={`rounded-full px-2.5 py-1 text-xs font-bold ${rs.bg} ${rs.color}`}
                          >
                            {rs.label}
                          </span>
                        )
                    ...
                    <div className="border-t border-slate-100 p-5 dark:border-slate-800">
                      <div className="mb-4 flex flex-wrap gap-2">
                        <Link
                          ...
                          className="bg-aerojet-blue inline-flex items-center gap-2 rounded-xl px-4 py-2 text-xs font-bold text-white hover:bg-blue-700"
                        >
                      ...
                      <div className="grid gap-4 sm:grid-cols-2">
                        <div>
                          ...
                          <div className="rounded-lg bg-slate-50 p-3 dark:bg-slate-800">
                            ...
                            <p className="mt-1 text-xs text-slate-500">
                              {bank.poolHealth.questionCount} questions /{' '}
                              {bank.poolHealth.requiredMinimum} required (5× exam size)
                            </p>
                          </div>
                        </div>
                        <div>
                          <div className="mb-2 flex items-center justify-between">
                            ...
                            <button
                              onClick={() => {
                                ...
                                setRulesForm({
                                  passMarkPct: (bank.ruleOverride?.passMarkPct ??
                                    ACADEMIC_RULES.EASA_PASS_MARK) as number,
                                  timePerQuestionSecs: (bank.ruleOverride?.timePerQuestionSecs ??
                                    ACADEMIC_RULES.TIME_PER_QUESTION_SECS) as number,
                                  retakeWaitDays: bank.ruleOverride?.retakeWaitDays ?? 90,
                                  maxRetakes: bank.ruleOverride?.maxRetakes ?? 3,
                                  completionWindowYears:
                                    bank.ruleOverride?.completionWindowYears ?? 10,
                                  allowKeyboardAutoSubmit:
                                    bank.ruleOverride?.allowKeyboardAutoSubmit ?? true,
                          ...
                          <div className="space-y-1 text-sm">
                            <div className="flex justify-between">
                              ...
                              <span className="font-medium">
                                {bank.ruleOverride?.passMarkPct ?? ACADEMIC_RULES.EASA_PASS_MARK}%
                              </span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-slate-500">Time/Question</span>
                              <span className="font-medium">
                                {bank.ruleOverride?.timePerQuestionSecs ??
                                  ACADEMIC_RULES.TIME_PER_QUESTION_SECS}
                                s
                              </span>
                            </div>
                            <div className="flex justify-between">
                              ...
                              <span className="font-medium">
                                {bank.ruleOverride?.retakeWaitDays ?? 90} days
                              </span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-slate-500">Max Attempts</span>
                              <span className="font-medium">
                                {bank.ruleOverride?.maxRetakes ?? 3}
                              </span>
                            </div>
                          </div>
                        </div>
                        <div className="sm:col-span-2">
                          ...
                          <div className="flex flex-col gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3 sm:flex-row sm:items-center sm:justify-between dark:border-slate-700 dark:bg-slate-800">
                            <p className="text-sm text-slate-600 dark:text-slate-300">
                              Share this link with students enrolled in {bank.course.code}. Portal
                              role and enrollment checks still apply before a student can start.
                            </p>
                            <button
                              type="button"
                        ...
                        {bank.pendingCount > 0 && (
                          <div className="mt-4 sm:col-span-2">
                            <h4 className="mb-2 text-xs font-bold tracking-widest text-indigo-600 uppercase dark:text-indigo-400">
                              Pending Review ({bank.pendingCount})
                            </h4>
                            <ApprovalQueue bankId={bank.id} />
                          </div>
                        )}

                        <div className="mt-4 flex justify-end border-t border-slate-100 pt-4 sm:col-span-2 dark:border-slate-800">
                          <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-slate-700 dark:bg-slate-900">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Configure exam rules. EASA standard is 75s/question — reduce to sharpen students for
              final exams.
            </p>
            ...
            <div className="mt-6 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setRulesForm({ ...rulesForm, timePerQuestionSecs: Number(e.target.value) })
                  }
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-800 dark:text-white"
                />
                <p className="mt-1 text-xs text-slate-400">
                  EASA standard: 75s. Lower = more challenging.
                </p>
              </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-black text-slate-900 dark:text-white">
                      {Math.round((rulesForm.timePerQuestionSecs * rulesForm.mcqCount) / 60)}{' '}
                      minutes
                    </p>
                    <p className="text-xs text-slate-500">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setRulesForm({ ...rulesForm, passMarkPct: Number(e.target.value) })
                  }
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-800 dark:text-white"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setRulesForm({ ...rulesForm, retakeWaitDays: Number(e.target.value) })
                  }
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-800 dark:text-white"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setRulesForm({ ...rulesForm, maxRetakes: Number(e.target.value) })
                  }
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-800 dark:text-white"
                />
              </div>
              ...
              <div>
                <label className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300">
                  <input
                    ...
                    onChange={(e) =>
                      setRulesForm({ ...rulesForm, allowKeyboardAutoSubmit: e.target.checked })
                    }
                ...
                onClick={async () => {
                  ...
                  try {
                    const res = await fetch(
                      `/api/staff/exams/internal/banks/${editingRulesBankId}/rule-override`,
                      {
                        method: 'PUT',
            ...
            <div className="mt-6 space-y-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setCreateForm({ ...createForm, courseId: e.target.value, moduleCode: '' })
                  }
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-800 dark:text-white"
                >
              ...
              <div>
                ...
                {!createForm.courseId && (
                  <p className="mt-1 text-xs text-slate-400">
                    Select a course first to see its modules.
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setCreateForm({ ...createForm, mcqCount: Number(e.target.value) })
                  }
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-800 dark:text-white"
                />
                <p className="mt-1 text-xs text-slate-400">
                  Recommended minimum: {createForm.mcqCount * 5} questions in pool (5× exam size for{' '}
                  {createForm.ruleSet} standard).
                </p>
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setCreateForm({ ...createForm, ruleSet: e.target.value as 'EASA' | 'CUSTOM' })
                  }
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-800 dark:text-white"
                >
