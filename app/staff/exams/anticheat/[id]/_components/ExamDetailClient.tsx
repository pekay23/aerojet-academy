...
export function ExamDetailClient({ examId }: { examId: string }) {
  ...
  const fetchExam = useCallback(async () => {
    ...
    try {
      ...
      if (sessionsJson.success)
        setSessions(
          Array.isArray(sessionsJson.data) ? sessionsJson.data : sessionsJson.data.sessions || []
        )
    } catch {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-8">
        <div className="border-aerojet-blue h-8 w-8 animate-spin rounded-full border-4 border-t-transparent" />
      </div>
    )
  }
  ...
  const tabs: {
    id: 'overview' | 'sessions' | 'questions'
    label: string
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-2 text-sm text-slate-500">
            <button
              onClick={() => router.push('/staff/exams/anticheat')}
              className="hover:text-aerojet-blue"
            >
              Exams
            </button>
        ...
        <div className="flex items-center gap-3">
          <span
            className={`rounded-full px-3 py-1 text-sm font-bold ${exam.status === 'active' ? 'bg-green-100 text-green-800' : exam.status === 'draft' ? 'bg-slate-100 text-slate-800' : 'bg-red-100 text-red-800'}`}
          >
            {exam.status}
          </span>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="bg-aerojet-blue flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-bold text-white hover:bg-blue-700"
            >
          ...
          {sessions.length === 0 ? (
            <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center dark:border-slate-800 dark:bg-slate-900">
              ...
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-500">
                Create a session to generate access codes for candidates.
              </p>
            </div>
          ) : (
            <div className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800">
              <table className="min-w-full divide-y divide-slate-200 bg-white dark:divide-slate-800 dark:bg-slate-900">
                <thead className="bg-slate-50 dark:bg-slate-800">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-bold tracking-wider text-slate-500 uppercase dark:text-slate-400">
                      Start Time
                    </th>
                    <th className="px-6 py-3 text-left text-xs font-bold tracking-wider text-slate-500 uppercase dark:text-slate-400">
                      End Time
                  ...
                  {sessions.map((s) => (
                    <tr key={s.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                      ...
                      <td className="px-6 py-4 text-sm">
                        <span
                          className={`rounded-full px-2 py-1 text-xs font-bold ${s.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-slate-100 text-slate-800'}`}
                        >
                          {s.status}
                        </span>
                      </td>
      ...
      {activeTab === 'questions' && (
        <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center dark:border-slate-800 dark:bg-slate-900">
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-500">
            Questions will be importable via the shared question import pipeline.
          </p>
        </div>
      )}
    </div>
