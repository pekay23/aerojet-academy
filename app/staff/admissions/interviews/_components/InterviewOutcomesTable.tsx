...
import { CheckCircle2, XCircle, AlertCircle, Clock, Calendar, HelpCircle } from 'lucide-react'
...
export default function InterviewOutcomesTable({ data }: { data: any[] }) {
  ...
  const handleOutcome = async (applicationId: string, outcome: 'PASS' | 'FAIL' | 'CONDITIONAL' | 'NO_SHOW') => {
    let notes = ''
    let scoreStr = ''
    ...
    if (outcome === 'FAIL' || outcome === 'NO_SHOW' || outcome === 'CONDITIONAL') {
      ...
      if (input === null && outcome !== 'CONDITIONAL') return // Require notes except if they cancel conditional maybe? Wait, prompt can be empty
      notes = input || ''
    } else {
            ...
            data.map((row) => (
              <tr key={row.id} className="transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/50">
                <td className="px-4 py-4">
                  <div className="font-bold text-slate-900 dark:text-white">{row.applicantName}</div>
                ...
                <td className="px-4 py-4">
                  {row.slot ? (
                    ...
                    <span className="text-xs font-semibold text-amber-600 dark:text-amber-400">Not Booked Yet</span>
                  )}
                </td>
                <td className="px-4 py-4">
                  <span className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs font-semibold ${
                    row.stage === 'INTERVIEW_COMPLETED' ? 'bg-teal-50 text-teal-700 dark:bg-teal-900/30 dark:text-teal-400' :
                    row.stage === 'INTERVIEW_SCHEDULED' ? 'bg-cyan-50 text-cyan-700 dark:bg-cyan-900/30 dark:text-cyan-400' :
                ...
                <td className="px-4 py-4 text-right">
                  {row.slot && row.stage !== 'INTERVIEW_COMPLETED' ? (
                    ...
                    <span className="text-xs text-slate-400">
                      {row.stage === 'INTERVIEW_COMPLETED' ? 'Outcome Pending' : 'Waiting for Booking'}
                    </span>
                  )}
                </td>
              </tr>
