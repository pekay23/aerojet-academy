...
export default function ImportWizard({ customFields }: { customFields: CustomFieldDefinition[] }) {
  ...
  const [validationResults, setValidationResults] = useState<
    { row: number; valid: boolean; data?: Record<string, unknown>; errors?: string[] }[]
  >([])
  const [isImporting, setIsImporting] = useState(false)
  const [importStats, setImportStats] = useState<{ imported: number; errors: string[] } | null>(
    null
  )
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Papa.parse<Record<string, unknown>>(file, {
      ...
      complete: (results) => {
        ...
        const lowerHeaders = (results.meta.fields || []).map((h) => ({
          original: h,
          lower: h.toLowerCase(),
        }))
    ...
    <div className="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <div className="border-b border-slate-100 p-4 dark:border-slate-800">
        <div className="flex items-center justify-between text-sm font-medium text-slate-500">
          <span
            className={step === 'UPLOAD' ? 'text-aerojet-blue dark:text-aerojet-sky font-bold' : ''}
          >
            1. Upload
          </span>
          <ArrowRight className="h-4 w-4" />
          <span
            className={step === 'MAP' ? 'text-aerojet-blue dark:text-aerojet-sky font-bold' : ''}
          >
            2. Map Columns
          </span>
          <ArrowRight className="h-4 w-4" />
          <span
            className={
              step === 'VALIDATE' ? 'text-aerojet-blue dark:text-aerojet-sky font-bold' : ''
            }
          >
            3. Validate
          </span>
          <ArrowRight className="h-4 w-4" />
          <span
            className={step === 'IMPORT' ? 'text-aerojet-blue dark:text-aerojet-sky font-bold' : ''}
          >
            4. Result
          </span>
        </div>
        ...
        {step === 'UPLOAD' && (
          <div className="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-300 bg-slate-50 py-16 text-center dark:border-slate-700 dark:bg-slate-800/50">
            ...
            <p className="mb-6 max-w-sm text-sm text-slate-500">
              Ensure your file has a header row. Legacy data will automatically create applications
              and user profiles.
            </p>
            <input
              type="file"
            ...
            <button
              ...
              className="bg-aerojet-blue hover:bg-aerojet-blue/90 rounded-lg px-6 py-2.5 text-sm font-bold text-white"
            >
              Select CSV File
            </button>
          </div>
        ...
        {step === 'MAP' && (
          <div className="space-y-6">
            ...
            <p className="text-sm text-slate-500">
              Match the fields in your CSV to the system properties.
            </p>
            ...
            <div className="flex justify-end border-t border-slate-100 pt-4 dark:border-slate-800">
              <button
                ...
                className="bg-aerojet-blue hover:bg-aerojet-blue/90 rounded-lg px-6 py-2 font-bold text-white disabled:opacity-50"
              >
        ...
        {step === 'VALIDATE' && (
          <div className="space-y-6">
            ...
            <div className="flex gap-4">
              <div className="flex-1 rounded-lg border border-green-200 bg-green-50 p-4 text-green-800 dark:border-green-800/50 dark:bg-green-900/20 dark:text-green-300">
                <div className="text-xl font-bold">
                  {validationResults.filter((r) => r.valid).length}
                </div>
                <div className="text-sm">Ready to Import</div>
              </div>
              <div className="flex-1 rounded-lg border border-red-200 bg-red-50 p-4 text-red-800 dark:border-red-800/50 dark:bg-red-900/20 dark:text-red-300">
                <div className="text-xl font-bold">
                  {validationResults.filter((r) => !r.valid).length}
                </div>
                ...
                <ul className="list-disc space-y-1 pl-5 text-red-700 dark:text-red-300">
                  ...
                  {validationResults.filter((r) => !r.valid).length > 50 && (
                    <li className="font-medium text-red-800">
                      + {validationResults.filter((r) => !r.valid).length - 50} more errors
                    </li>
                  )}
                </ul>
              </div>
