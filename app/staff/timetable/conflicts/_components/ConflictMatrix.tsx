...
export default function ConflictMatrix({
  ...
  const [kind, setKind] = useState<'ALL' | 'INSTRUCTOR' | 'CLASSROOM' | 'INSTRUCTOR_UNAVAILABLE'>(
    'ALL'
  )
  ...
  const resourceMap = useMemo(() => {
    const m = new Map<
      string,
      { kind: ConflictDto['kind']; label: string; perDay: Record<string, ConflictDto[]> }
    >()
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-end gap-2 rounded-2xl border border-slate-100 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
        <div>
          <label
            htmlFor="conflict-from"
            className="block text-[10px] font-black tracking-widest text-slate-400 uppercase"
          >
            From
          </label>
          <input
        ...
        <div className="ml-auto text-sm text-slate-500">
          <span className="font-black text-red-600">{filtered.length}</span> conflict pair
        ...
        <div className="overflow-x-auto rounded-2xl border border-slate-100 bg-white dark:border-slate-800 dark:bg-slate-900">
          <table className="min-w-full text-sm">
            <thead className="bg-slate-50 text-left text-[10px] font-black tracking-widest text-slate-400 uppercase dark:bg-slate-800">
              <tr>
                <th className="sticky left-0 z-10 bg-slate-50 px-3 py-2 dark:bg-slate-800">
                  Resource
                </th>
                {days.map((d) => (
                        ...
                        <div className="group relative inline-flex h-6 w-6 cursor-default items-center justify-center rounded-md bg-red-100 text-[10px] font-black text-red-700 hover:bg-red-200">
                          ...
                          <div className="invisible absolute bottom-full left-1/2 z-20 mb-1 w-64 -translate-x-1/2 rounded-lg bg-slate-900 p-3 text-left text-xs text-white shadow-lg group-hover:visible">
                            <p className="mb-1 font-black">
                              <AlertOctagon className="inline h-3 w-3" /> {here.length} conflict
                              {here.length === 1 ? '' : 's'}
                            </p>
                            {here.slice(0, 3).map((c, i) => (
                              <p key={i} className="opacity-90">
                                {c.a.className} ↔ {c.b.className} (
                                {new Date(c.a.start).toLocaleTimeString([], {
                                  hour: '2-digit',
                                  minute: '2-digit',
                                })}
                                )
                            ...
                            {here.length > 3 && (
                              <p className="mt-1 italic opacity-60">+{here.length - 3} more</p>
                            )}
                          </div>
                        </div>
                      </td>
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-100 bg-white dark:border-slate-800 dark:bg-slate-900">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.slice(0, 100).map((c, i) => (
                <tr
                  key={`${c.kind}-${c.resourceId}-${i}`}
                  className="border-t border-slate-100 dark:border-slate-800"
                >
                  <td className="px-3 py-2 text-xs">
                    <span
                      className={`rounded-full px-2 py-0.5 text-[10px] font-black tracking-widest uppercase ${c.kind === 'INSTRUCTOR' ? 'bg-indigo-50 text-indigo-700' : c.kind === 'CLASSROOM' ? 'bg-amber-50 text-amber-700' : 'bg-red-50 text-red-700'}`}
                    >
                      {c.kind}
                    </span>
                  </td>
                  ...
                  <td className="px-3 py-2">
                    <a
                      href={`/staff/classes/${c.a.classId}`}
                      className="text-aerojet-blue hover:underline"
                    >
                      {c.a.className}
                    </a>
                    <p className="text-[10px] text-slate-400">
                      ...
                      →{' '}
                      {new Date(c.a.end).toLocaleTimeString([], {
                        hour: '2-digit',
                        minute: '2-digit',
                      })}
                    </p>
                  ...
                  <td className="px-3 py-2">
                    <a
                      href={`/staff/classes/${c.b.classId}`}
                      className="text-aerojet-blue hover:underline"
                    >
                      {c.b.className}
                    </a>
                    <p className="text-[10px] text-slate-400">
                      {new Date(c.b.start).toLocaleTimeString([], {
                        hour: '2-digit',
                        minute: '2-digit',
                      })}{' '}
                      →{' '}
                      {new Date(c.b.end).toLocaleTimeString([], {
                        hour: '2-digit',
                        minute: '2-digit',
                      })}
                    </p>
          ...
          {filtered.length > 100 && (
            <div className="px-3 py-2 text-center text-xs text-slate-400">
              Showing first 100. Narrow the date range to see more.
            </div>
          )}
        </div>
      )}
