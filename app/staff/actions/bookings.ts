...
async function resolveTargetStudent(
  ...
  if (targetUserId) {
    const profile = await prisma.profile.findUnique({ where: { userId: targetUserId }, select: { firstName: true, lastName: true } })
...
async function finalizeExamUpdate(
  ...
  await createAuditLog({
    ...
    changes: {
      ...
      resultLocked: overrides?.resultLocked ?? (isResultId ? (data.supervisorOverrideJustification ? false : undefined) : undefined),
      supervisorOverride: overrides?.supervisorOverride ?? data.supervisorOverrideJustification || undefined,
  ...
  if (resultIdToSync) {
    await tx.examResult.update({
      ...
      data: {
        ...
        attemptType: attemptType || booking.attemptType,
        examCategory: (examCategory as ExamCategory) || booking.examCategory,
        moduleCode: finalModule,
        migrationRef: isMigrated !== undefined
          ? (isMigrated ? (migrationRef || 'MANUAL_CORRECTION') : null)
          : undefined,
      },
    })
  } else {
...
export async function updateExamBooking(
  ...
  try {
    ...
    const { userId, name: targetStudentName } = await resolveTargetStudent(
      prismaUnfiltered, bookingId, false,
    )

    return finalizeExamUpdate(
      staff, bookingId, false, userId, targetStudentName, data, moduleCode, score,
    )
  } catch (error) {
...
export async function updateExamResult(
  ...
  try {
    ...
    const { userId, name: targetStudentName } = await resolveTargetStudent(
      prismaUnfiltered, resultId, true,
    )

    return finalizeExamUpdate(
      staff, resultId, true, userId, targetStudentName, data, moduleCode, score,
      {
