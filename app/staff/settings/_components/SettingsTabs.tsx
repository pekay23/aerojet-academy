...
export default function SettingsTabs({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Tab Bar */}
      <div className="relative flex flex-wrap gap-1 rounded-2xl bg-slate-100 p-1.5 shadow-inner ring-1 ring-black/5 dark:bg-slate-800/80 dark:ring-white/5">
        {TABS.map((t) => {
          const Icon = t.icon
          ...
          return (
            <button
              ...
              className={`relative flex items-center gap-1.5 rounded-full px-3 py-2 text-xs font-bold transition-colors duration-150 ${
                isActive
