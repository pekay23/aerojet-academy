    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 rounded-2xl bg-white p-4 shadow-sm border border-slate-100 dark:bg-slate-900 dark:border-slate-800">
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setCurrentDate(new Date())} className="text-xs font-bold text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors px-3 py-1.5 rounded-full border border-slate-200 dark:border-slate-700">Today</button>
        </div>
        <div className="flex gap-2">
          <div className="flex items-center rounded-full border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
            <button onClick={() => setViewMode('Month')} className={cn('px-4 py-2.5 text-sm font-bold rounded-l-full transition-colors', viewMode === 'Month' ? 'bg-slate-100 text-slate-900 dark:bg-slate-700 dark:text-white' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400')}>Month</button>
            <button onClick={() => setViewMode('Week')} className={cn('px-4 py-2.5 text-sm font-bold transition-colors border-l border-slate-200 dark:border-slate-700', viewMode === 'Week' ? 'bg-slate-100 text-slate-900 dark:bg-slate-700 dark:text-white' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400')}>Week</button>
            <button onClick={() => setViewMode('Day')} className={cn('px-4 py-2.5 text-sm font-bold rounded-r-full transition-colors border-l border-slate-200 dark:border-slate-700', viewMode === 'Day' ? 'bg-slate-100 text-slate-900 dark:bg-slate-700 dark:text-white' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400')}>Day</button>
          </div>
                  ...
                  <div className="space-y-2 p-2">
                    {hourEvents.map((evt) => (
                      <button
                        ...
                        className={cn('w-full rounded-xl p-3 text-left text-sm font-bold shadow-sm', getEventStyles(evt.source, evt.color))}
                        style={evt.source === 'admin' ? { backgroundColor: evt.color, color: '#fff' } : undefined}
                      >
                        <span className="block truncate">{evt.title}</span>
                        <span className="block text-xs font-medium opacity-80">{format(new Date(evt.startDate), 'hh:mm a')}{evt.endDate ? ` - ${format(new Date(evt.endDate), 'hh:mm a')}` : ''}</span>
                      </button>
                    ))}
                  </div>
                </div>
      ...
      {/* Week View */}
      {viewMode === 'Week' && (
        <div className="hidden md:flex flex-col bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden dark:bg-slate-900 dark:border-slate-800">
