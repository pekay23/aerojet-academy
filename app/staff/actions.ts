...
export async function markMessageAsRead(messageId: string) {
  try {
    ...
    return {
      error: handleActionError('markMessageAsRead', error, 'Failed to mark message as read.'),
    }
  }
}
...
export async function bulkDeleteUsers(userIds: string[]) {
  try {
    ...
    return {
      error: handleActionError('bulkDeleteUsers', error, 'Failed to delete users permanently.'),
    }
  }
}
...
export async function bulkUpdatePaymentStatus(paymentIds: string[], status: PaymentStatus) {
  try {
    ...
    return {
      error: handleActionError('bulkUpdatePaymentStatus', error, 'Failed to update payments.'),
    }
  }
}
...
export async function bulkBypassPasswordChange(userIds: string[]) {
  try {
    ...
    return {
      error: handleActionError('bulkBypassPasswordChange', error, 'Failed to update users.'),
    }
  }
}
/**
...
export async function updateExamBooking(
  ...
  try {
            ...
            data: {
              ...
              (score !== undefined ||
                ...
                ? {
                    ...
                    percentage:
                      score ?? (booking.score != null ? Number(booking.score) : undefined),
                    result:
          ...
          if (
            ...
            if (data.resultIdToSync) {
              ...
              await tx.examResult.update({
                ...
                data: {
                  ...(score !== undefined
                    ? { score, maxScore: 100, percentage, passed, grade }
                    : {}),
                  ...
                  migrationRef:
                    data.isMigrated !== undefined
                      ? data.isMigrated
                        ? data.migrationRef || 'MANUAL_CORRECTION'
                        : null
                      : undefined,
                },
              })
    ...
    if (isResultId) {
      ...
      const b = await prismaUnfiltered.examBooking.findUnique({
        where: { id: actualId },
        select: { userId: true },
      })
    ...
    if (targetUserId) {
      const profile = await prismaUnfiltered.profile.findUnique({
  ...
  try {
    ...
    return {
      error: handleActionError(
        ...
        error,
        'Failed to update certificate release settings.'
      ),
    }
  }
}
