...
export default function TestInterface({ session }: TestInterfaceProps) {
  ...
  const handleAnswer = async (questionId: string, answer: string) => {
    // Optimistic update
    setQuestions(prev => prev.map(q => q.questionId === questionId ? { ...q, selectedAnswer: answer } : q))
  ...
  const answeredCount = questions.filter(q => q.selectedAnswer !== null && q.selectedAnswer !== '').length
      ...
      <header className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-200 bg-white px-6 py-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex items-center gap-4">
          <div className="h-2 w-24 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
            <div 
              className="h-full bg-aerojet-blue transition-all" 
              style={{ width: `${(answeredCount / questions.length) * 100}%` }}
            />
          </div>
          <span className="text-xs font-bold text-slate-500">{answeredCount} of {questions.length} answered</span>
        </div>
            ...
            {questions.map((q, i) => (
              <button
                ...
                className={`h-2.5 w-2.5 rounded-full transition-all ${
                  currentIndex === i ? 'bg-aerojet-blue scale-125' : 
                  q.selectedAnswer ? 'bg-slate-400' : 'bg-slate-200 dark:bg-slate-700'
                }`}
                title={`Question ${i + 1}`}
              />
            ))}
