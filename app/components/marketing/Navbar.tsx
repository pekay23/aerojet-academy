...
const navLinks = {
  ...
  courses: {
    aircraftEngineering: {
      ...
      href: "/courses/aircraft-engineering",
      sublinks: [
        { title: "EASA Part-66 Programs", href: "/courses/aircraft-engineering/easa-part-66-programs" },
        { title: "Other Programs", href: "/courses/aircraft-engineering/other-programs" },
        { title: "Short Knowledge Courses", href: "/courses/aircraft-engineering/short-courses" },
      ]
    },
    otherProgrammes: [
        ...
        <div className="flex flex-col space-y-3 pl-4 border-l-2 border-gray-100 ml-1">
          {isGrouped ? (
            <>
              ...
              <Link href={links.aircraftEngineering.href} onClick={onLinkClick} className="block text-sm font-bold text-aerojet-blue hover:text-aerojet-sky py-1">
                {links.aircraftEngineering.title}
              </Link>
              <div className="pl-3 space-y-2 border-l border-gray-200">
                {links.aircraftEngineering.sublinks.map((link: any) => (
                  <Link key={link.title} href={link.href} onClick={onLinkClick} className="block text-xs text-gray-600 hover:text-aerojet-sky font-medium py-1">{link.title}</Link>
                ))}
              </div>

              <div className="text-xs font-bold text-gray-400 uppercase tracking-wider mt-4">Other Programmes</div>
              {links.otherProgrammes.map((link: any) => (
            ...
            <div className="group relative h-full flex items-center">
              ...
              <div className="absolute top-full left-1/2 -translate-x-1/2 pt-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none group-hover:pointer-events-auto">
                <div className="bg-white rounded-md shadow-lg w-72 p-4 border border-gray-100 text-left">
                  ...
                  {/* Aircraft Engineering Section */}
                  <Link href={navLinks.courses.aircraftEngineering.href} className="block px-2 py-2 text-sm font-bold text-aerojet-blue hover:bg-gray-50 rounded-md mb-1">
                    {navLinks.courses.aircraftEngineering.title}
                  </Link>
                  <div className="pl-3 mb-3 border-l-2 border-gray-100">
                    {navLinks.courses.aircraftEngineering.sublinks.map(link => (
                      <Link key={link.title} href={link.href} className="block px-2 py-1.5 text-xs text-gray-600 hover:bg-gray-50 hover:text-aerojet-blue rounded-md">{link.title}</Link>
                    ))}
                  </div>

                  <div className="text-xs font-bold text-gray-400 uppercase tracking-wider mt-4 mb-2 border-b pb-1">Other Programmes</div>
                  {navLinks.courses.otherProgrammes.map(link => (
