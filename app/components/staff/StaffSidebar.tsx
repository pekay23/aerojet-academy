...
const allMenuItems = [
  ...
  { title: "Results", href: "/staff/results", icon: Icons.Results, roles: ['ADMIN', 'STAFF', 'INSTRUCTOR'] }, 
    ...
    <>
      <aside className={`bg-slate-900 text-white flex flex-col h-full shadow-xl transition-all duration-300 relative ${collapsed ? 'w-20' : 'w-64'}`}>
        ...
        <div className="h-16 flex items-center justify-center border-b border-white/10 mx-4 shrink-0">
          <Link href="/">{collapsed ? <span className="font-bold text-xl">AA</span> : <Image src="/ATA_logo_hor_onDark.png" alt="Aerojet Academy" width={140} height={35} className="object-contain" />}</Link>
        </div>
        
        <div className={`py-6 flex flex-col items-center justify-center shrink-0 transition-all duration-300 ${collapsed ? 'px-2' : 'px-4'}`}>
          <div className={`rounded-full bg-slate-700 flex items-center justify-center font-bold text-white transition-all mx-auto overflow-hidden ${collapsed ? 'w-10 h-10 text-sm' : 'w-16 h-16 text-xl'}`}>
            {user.image ? <Image src={user.image} alt="User" fill className="object-cover" /> : user.name?.[0]}
          </div>
          {!collapsed && (
