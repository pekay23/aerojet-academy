...
export const PATCH = withErrorHandler(
  async (req: NextRequest, ctx: { params: Promise<{ bankId: string; scheduleId: string }> }) => {
...
export const DELETE = withErrorHandler(async (req: NextRequest, ctx: RouteContext) => {
  ...
  const schedule = await prismaUnfiltered.internalExamClassSchedule.findUnique({
    where: { id: scheduleIdStr },
  })
  if (!schedule || schedule.bankId !== bankIdStr) {
    return apiError('Schedule not found', 404)
  }
