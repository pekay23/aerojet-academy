...
export const PATCH = withErrorHandler(async (req: NextRequest, ctx: { params: Promise<{ bankId: string; instructorId: string }> }) => {
...
export const DELETE = withErrorHandler(async (req: NextRequest, ctx: { params: Promise<{ bankId: string; instructorId: string }> }) => {
  const staff = await requireStaff()
  if (!(await isInternalExamSystemEnabled())) {
    return apiError('Internal exams are not currently available', 403)
  }
