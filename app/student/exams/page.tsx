...
export default async function ExamsPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-black tracking-tight text-slate-900 dark:text-slate-100 sm:text-3xl">My Exams</h1>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
        ...
        {bookings.length > 0 ? (
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-50">
                {bookings.map((booking) => (
                  <tr key={booking.id} className="transition-colors hover:bg-slate-50 dark:bg-slate-800/50">
                    <td className="px-6 py-4">
                      <div className="font-bold text-slate-900 dark:text-slate-100">
                        {booking.exam?.course.name || booking.moduleCode}
                      </div>
          ...
          <div className="p-8 text-center italic text-slate-500 dark:text-slate-400">No bookings found.</div>
        )}
      </div>
        ...
        {results.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-50">
                {results.map((result) => (
                  <tr key={result.id} className="transition-colors hover:bg-slate-50 dark:bg-slate-800/50">
                    <td className="px-6 py-4">
                      <div className="font-bold text-slate-900 dark:text-slate-100">{result.exam.course.name}</div>
                    ...
                    <td className="whitespace-nowrap px-6 py-4 text-right text-slate-500 dark:text-slate-400">
          ...
          <div className="p-8 text-center font-medium italic text-slate-500 dark:text-slate-400">
