  ...
  return (
    <div className="space-y-8">
      ...
      {wallet && (
        ...
        >
          <div className="flex items-center gap-3">
            <div
              className={`rounded-lg p-2 ${balance > 0 ? 'bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400' : 'bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400'}`}
            >
      ...
      <div className="flex items-start gap-3 rounded-xl border border-blue-100 bg-blue-50/40 px-5 py-4 text-sm text-blue-800 dark:border-blue-900/50 dark:bg-blue-900/10 dark:text-blue-300">
        ...
        <span>
          <strong>Join Available Exam Pools</strong> to take advantage of the cheapest price of the
          exams (€300/seat). Please note that <strong>there is no refund</strong> until admin
          refunds the money (or only credits the wallet, not bank) because everyone in the pool is
          relying on everyone to make the class/seat/pool viable. Each pool accepts{' '}
          <strong>25–28 candidates</strong>.
        </span>
      </div>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {pools.map((pool) => {
            ...
            return (
              ...
              >
                <div
                  className={`flex items-center justify-between px-6 py-2 text-xs font-black tracking-widest text-white uppercase ${displayStatus === 'NEAR_FULL' ? 'bg-amber-500' : displayStatus === 'OPEN' ? 'bg-blue-800' : displayStatus === 'CONFIRMED' ? 'bg-blue-600' : 'bg-slate-500'}`}
                >
                  <span>{displayStatus.replace('_', ' ')}</span>
                  {pool.timeSlot && (
                    <span className="text-white/80">
                      {pool.poolLabel ? `Pool ${pool.poolLabel}` : ''} · Day ${pool.dayNumber} $
                      {pool.timeSlot === 'MORNING' ? 'AM' : 'PM'}
                    </span>
                  )}
                </div>
                <div className="flex flex-1 flex-col p-6">
                  ...
                  <div className="mb-4 space-y-3">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="h-1.5 w-full overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
                        <div
                          className={`h-full transition-all duration-500 ${pool.status === 'NEAR_FULL' ? 'bg-amber-500' : pool.status === 'CONFIRMED' ? 'bg-emerald-500' : 'bg-blue-500'}`}
                          style={{
                  ...
                  {existingModules.length > 0 && (
                    <div className="mb-4 rounded-lg border border-slate-100 bg-slate-50 px-3 py-2 dark:border-slate-800 dark:bg-slate-800/50">
                      <p className="mb-1.5 text-xs font-bold tracking-widest text-slate-400 uppercase">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {existingModules.map((m) => (
                          <span
                            key={m}
                            className="inline-flex rounded-md bg-blue-800/10 px-2 py-0.5 text-xs font-bold text-blue-800 dark:bg-blue-900/30 dark:text-blue-300"
                          >
                            {m}
                          </span>
                        ))}
                  ...
                  {surcharge > 0 && (
                    <div className="mb-4 flex items-start gap-1.5 rounded-lg border border-red-200 bg-red-50 p-2.5 text-[10px] font-bold tracking-wide text-red-700 uppercase dark:border-red-900/50 dark:bg-red-900/20 dark:text-red-400">
                      <AlertCircle className="mt-0.5 h-3.5 w-3.5 shrink-0" />
                      <span>
                        {currencySymbol}
                        {surcharge} Late Booking Surcharge Applied (T-
                        {bookingData.pricing.lateBookingDays})
                      </span>
                    </div>
                  )}
