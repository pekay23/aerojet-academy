...
export default async function RegistrationFeePage() {
  ...
  const currency = user.registrationCurrency
  const symbol = getCurrencySymbol(currency)
  const available = wallet ? Number(wallet.availableBalance) : 0
  const canPay = available >= fee

  // Already paid — success state
  if (user.registrationPaid) {
    return (
      <div className="mx-auto max-w-lg space-y-6">
        <h1 className="text-2xl font-black text-[#002a5c] dark:text-white">Registration Fee</h1>
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-8 text-center dark:border-emerald-800/50 dark:bg-emerald-900/10">
          ...
          <p className="mb-1 text-sm text-emerald-700 dark:text-emerald-300">
            Amount: {symbol}{fee.toFixed(2)} {currency}
          </p>
        ...
        <div className="text-center">
          <Link
            ...
            className="text-sm font-medium text-[#002a5c] underline hover:no-underline dark:text-blue-400"
          >
            Back to Dashboard
          </Link>
        </div>
    ...
    <div className="mx-auto max-w-lg space-y-6">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
        <div className="mb-4 flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#002a5c]/10 dark:bg-blue-500/10">
            <Info className="h-5 w-5 text-[#002a5c] dark:text-blue-400" />
          </div>
          <div>
            ...
            <p className="text-2xl font-black text-[#002a5c] dark:text-white">
              {symbol}{fee.toFixed(2)} <span className="text-sm font-medium text-slate-400">{currency}</span>
            </p>
          </div>
        </div>
      ...
      {/* Action area */}
      {canPay ? (
        <PayRegistrationFeeButton fee={fee} symbol={symbol} currency={currency} />
      ) : (
        <div className="space-y-4">
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-6 text-center dark:border-amber-800/50 dark:bg-amber-900/10">
            ...
            <p className="mb-4 text-sm text-amber-700 dark:text-amber-300">
              You need {symbol}{(fee - available).toFixed(2)} more to pay the registration fee.
            </p>
            <Link
              href="/student/wallet?action=topup"
              className="inline-flex items-center gap-2 rounded-xl bg-[#002a5c] px-6 py-2.5 text-sm font-bold text-white transition-colors hover:bg-[#001d42]"
            >
