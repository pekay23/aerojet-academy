...
export default async function StudentAcademicCalendarPage() {
  ...
  const [personalEvents, examBookings, enrollments, adminEvents, sittingAssignments] =
    await Promise.all([
  ...
  examBookings.forEach((exam) => {
    ...
    if (exam.examDate) {
      events.push({
        id: `exam-${exam.id}`,
        dbId: exam.id,
        title: `Exam: ${exam.moduleCode || 'Module'}`,
  ...
  async function handleSave(data: Parameters<typeof createCalendarEvent>[0], editingId?: string) {
    'use server'
    return editingId ? updateCalendarEvent(editingId, data) : createCalendarEvent(data)
  }
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 mx-auto max-w-350 space-y-8 duration-700">
      <div>
        <h1 className="text-3xl font-black tracking-tight text-blue-800 uppercase dark:text-white">
          Academic Calendar
        </h1>
