...
export default async function Page() {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 space-y-6 duration-700">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="group relative overflow-hidden rounded-3xl border border-slate-100 bg-white p-6 transition-all hover:shadow-xl hover:shadow-blue-500/10 dark:border-slate-800 dark:bg-slate-900">
          ...
          <div className="relative z-10 mt-5">
            ...
            <p className="mt-1 text-xs font-bold tracking-widest text-slate-400 uppercase dark:text-slate-300 dark:text-slate-500">
              Active Classes
            </p>
          </div>
        </div>
        ...
        <div className="overflow-hidden rounded-3xl border border-slate-100 bg-white lg:col-span-2 dark:border-slate-800 dark:bg-slate-900">
          <div className="flex flex-col gap-2 border-b border-slate-50 px-6 py-5 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Link
              ...
              className="group text-aerojet-blue flex items-center gap-1.5 text-xs font-black tracking-widest uppercase transition-all hover:translate-x-1"
            >
              Full Schedule{' '}
              ...
              instructorData.todaysClasses.map((cls) => (
                ...
                >
                  <div className="flex items-center gap-5">
                    <div className="text-aerojet-blue flex h-14 w-14 items-center justify-center rounded-2xl bg-blue-50 dark:bg-blue-900/20">
                      <Clock className="h-6 w-6" />
                    </div>
                    <div>
                      <p className="group-hover:text-aerojet-blue text-base font-black text-slate-900 transition-colors dark:text-slate-100">
                        {cls.course.name}
                      </p>
        ...
        <div className="flex flex-col overflow-hidden rounded-3xl border border-slate-100 bg-white dark:border-slate-800 dark:bg-slate-900">
          ...
          <div className="flex-1 divide-y divide-slate-50 dark:divide-slate-800">
            {instructorData.pendingGradingDetails.length > 0 ? (
              instructorData.pendingGradingDetails.map((item, i) => (
                <div
                  key={i}
                  className="px-6 py-5 transition-colors hover:bg-orange-50/30 dark:hover:bg-orange-950/10"
                >
                  <div className="mb-2 flex items-center justify-between">
                    <span className="text-aerojet-blue text-xs font-black">{item.module}</span>
          ...
          <div className="bg-slate-50 p-4 dark:bg-slate-800/50">
            <Link
              ...
              className="hover:bg-aerojet-blue flex h-11 items-center justify-center rounded-2xl bg-white text-xs font-black tracking-widest text-slate-600 uppercase shadow-sm transition-all hover:text-white hover:shadow-lg dark:bg-slate-900 dark:text-slate-400"
            >
              Enter Grading Hub
            </Link>
          </div>
            ...
            instructorData.recentNotices.map((notice) => (
              ...
              >
                <div className="mb-4">
                  ...
                  <p className="group-hover:text-aerojet-blue text-sm leading-snug font-black text-slate-800 transition-colors dark:text-slate-200">
                    {notice.title}
                  </p>
                </div>
      ...
      {/* Quick Actions */}
      <div className="rounded-3xl border border-slate-100 bg-white p-8 dark:border-slate-800 dark:bg-slate-900">
        <h2 className="mb-6 text-lg font-black text-slate-900 dark:text-white">Quick Actions</h2>
        <div className="flex flex-wrap gap-3">
          {[
            ...
            <Link
              key={action.label}
              href={action.href}
              className="hover:bg-aerojet-blue inline-flex items-center gap-1.5 rounded-2xl bg-slate-100 px-6 py-3 text-[10px] font-black tracking-widest text-slate-600 uppercase transition-all hover:text-white hover:shadow-lg hover:shadow-blue-500/20 dark:bg-slate-800 dark:text-slate-400"
            >
              {action.label}
            </Link>
          ))}
