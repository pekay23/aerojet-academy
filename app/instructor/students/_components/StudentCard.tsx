...
export default function StudentCard({ student }: { student: Student }) {
  ...
  return (
    <Link href={`/instructor/students/${student.id}`}>
      <motion.div
        whileHover={{ y: -4 }}
        className="group relative flex flex-col items-center overflow-hidden rounded-3xl border border-slate-100 bg-white p-6 shadow-sm transition-all hover:border-[#4c9ded]/30 hover:shadow-xl hover:shadow-blue-500/10 dark:border-slate-800 dark:bg-slate-900 dark:hover:border-[#4c9ded]/30"
      >
        {/* Background Accent */}
        ...
        <div className="text-center">
          <h3 className="line-clamp-1 text-lg font-black text-slate-900 transition-colors group-hover:text-[#4c9ded] dark:text-white">
            {student.name}
          </h3>
        ...
        <div className="mt-6 flex w-full items-center justify-between border-t border-slate-50 pt-4 dark:border-slate-800/50">
          <span className="text-[10px] font-black tracking-widest text-[#4c9ded] uppercase">
            View Profile
          </span>
          <ChevronRight className="h-4 w-4 text-slate-300 transition-transform group-hover:translate-x-1 group-hover:text-[#4c9ded]" />
        </div>
      </motion.div>
    </Link>
  )
