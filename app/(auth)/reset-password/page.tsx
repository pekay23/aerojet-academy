...
export default function ResetPasswordPage() {
  ...
  if (!token) {
    return (
      <div className="text-center">
        <h2 className="text-2xl font-black text-[#002a5c] uppercase tracking-tight mb-3">Invalid Link</h2>
        <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">This password reset link is invalid or has expired.</p>
        <Link href="/forgot-password" className="text-[#4c9ded] font-bold text-sm hover:underline">Request a new link</Link>
      </div>
    );
  }
  ...
  if (success) {
    return (
      <div className="text-center">
        ...
        <h2 className="text-2xl font-black text-[#002a5c] uppercase tracking-tight mb-3">Password Updated</h2>
        <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">You can now sign in with your new password.</p>
        <Link href="/login" className="inline-block bg-[#002a5c] text-white px-8 py-3 rounded-xl font-black uppercase text-xs tracking-widest hover:bg-[#4c9ded] transition-all">
          Sign In
        </Link>
      </div>
    );
  ...
  return (
    <div>
      <div className="mb-8">
        <h2 className="text-2xl font-black text-[#002a5c] uppercase tracking-tight">New Password</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <button type="submit" disabled={loading || password.length < 8} className="w-full bg-[#002a5c] text-white py-4 rounded-xl font-black uppercase tracking-widest text-xs hover:bg-[#4c9ded] transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg">
