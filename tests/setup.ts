...
vi.mock('@/lib/database/prisma', () => ({
  default: {
    user: { findUnique: vi.fn(), findMany: vi.fn(), create: vi.fn(), update: vi.fn(), count: vi.fn(), upsert: vi.fn() },
    ...
    poolMembership: { findFirst: vi.fn(), create: vi.fn(), count: vi.fn(), findMany: vi.fn(), update: vi.fn() },
    payment: { findUnique: vi.fn(), create: vi.fn(), update: vi.fn(), findMany: vi.fn(), count: vi.fn() },
...
// Mock resend
vi.mock('resend', () => ({ Resend: vi.fn(() => ({ emails: { send: vi.fn(() => ({ data: { id: 'test' }, error: null })) } })) }))
