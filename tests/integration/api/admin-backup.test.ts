...
vi.mock('@/lib/backup', () => ({
  BACKUP_MODELS: [{ key: 'user', label: 'Users' }],
  exportAllTables: vi.fn().mockResolvedValue({ user: [{ id: '1' }] }),
  generateJsonBackup: vi.fn().mockReturnValue({ tables: { user: [{ id: '1' }] } }),
  generateReadableReport: vi.fn().mockReturnValue('<html>report</html>'),
  sendBackupEmail: vi.fn().mockResolvedValue(undefined),
}))

import { GET } from '@/app/api/admin/backup/route.ts'
import { POST } from '@/app/api/admin/backup/route.ts'
import { getAuthSession, requireStaff, requireAdmin, requireAuth } from '@/lib/auth/helpers'

describe('GET/POST /api/admin/backup', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  describe('GET', () => {
    it('returns backup JSON by default', async () => {
    ...
    it('returns backup HTML when format=html', async () => {
      const req = new NextRequest('http://localhost/api/admin/backup?format=html', {
        headers: { Authorization: 'Bearer test-secret' },
      })
      const res = await GET(req)
      expect(res.status).toBe(200)
  ...
  describe('POST', () => {
    ...
    it('returns 200 on valid backup email request', async () => {
      const req = new NextRequest('http://localhost/api/admin/backup', {
        method: 'POST',
        headers: { Authorization: 'Bearer test-secret' },
