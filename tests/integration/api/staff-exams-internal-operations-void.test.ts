...
import { POST } from '@/app/api/staff/exams/internal/operations/void/route.ts'
import { requireStaff } from '@/lib/auth/helpers'

describe('/staff/exams/internal/operations/void', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    ;(requireStaff as any).mockResolvedValue({ id: 'user-1', role: 'ADMIN' })
    prismaMock.internalExamSession.findUnique.mockResolvedValue(null as any)
  })

  it('returns 401 when unauthenticated', async () => {
    ;(requireStaff as any).mockRejectedValueOnce(new Error('Unauthorized'))
    const req = new NextRequest('http://localhost/staff/exams/internal/operations/void', {
      method: 'POST',
      body: JSON.stringify({}),
    })
  ...
  it('returns 400 when session already voided', async () => {
    prismaMock.internalExamSession.findUnique.mockResolvedValueOnce({ id: 's1', status: 'VOIDED' } as any)
    const req = new NextRequest('http://localhost/staff/exams/internal/operations/void', {
      method: 'POST',
      body: JSON.stringify({ sessionId: 's1' }),
    })
  ...
  it('returns 200 for valid void', async () => {
    prismaMock.internalExamSession.findUnique.mockResolvedValueOnce({ id: 's1', status: 'COMPLETED', studentId: 'stu1', bankId: 'b1', categoryCode: 'CAT1' } as any)
    prismaMock.internalExamSession.update.mockResolvedValueOnce({ id: 's1', status: 'VOIDED' } as any)
    prismaMock.internalExamReport.updateMany.mockResolvedValueOnce({ count: 0 } as any)
    const req = new NextRequest('http://localhost/staff/exams/internal/operations/void', {
      method: 'POST',
      body: JSON.stringify({ sessionId: 's1' }),
    })
