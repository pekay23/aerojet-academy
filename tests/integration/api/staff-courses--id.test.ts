vi.mock('@/lib/auth/helpers', () => ({
 ...
 vi.mock('@/lib/validation/schemas', () => ({ updateCourseSchema: vi.fn(), validateBody: vi.fn() }))
vi.mock('@/lib/prisma/soft-delete', () => ({ softDeleteData: vi.fn().mockReturnValue({ success: true }) }))
...
vi.mock('@/lib/api/response', () => ({
 ...
 import { GET, PATCH, DELETE } from '@/app/api/staff/courses/[id]/route.ts'
 import { requireStaff } from '@/lib/auth/helpers'
 import { validateBody } from '@/lib/validation/schemas'


describe('/staff/courses/:id', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    ...
    ;(validateBody as any).mockReturnValue({ success: true, data: {} })
  })

  it('returns 401 when unauthenticated (GET)', async () => {
    ;(requireStaff as any).mockRejectedValueOnce(new Error('Unauthorized'))
    const req = new NextRequest('http://localhost/staff/courses/1')
    const res = await GET(req, { params: Promise.resolve({ id: '1' }) })
    expect(res.status).toBe(401)
  })

  it('returns 200 with data when course exists', async () => {
    prismaMock.course.findFirst.mockResolvedValueOnce({ id: '1' })
    prismaMock.course.findUnique.mockResolvedValueOnce({ id: '1', code: 'C1', name: 'Course 1', enrollments: [], classes: [], _count: { enrollments: 0, classes: 0, examComponents: 0 } } as any)
    const req = new NextRequest('http://localhost/staff/courses/1')
    const res = await GET(req, { params: Promise.resolve({ id: '1' }) })
  ...
  it('returns 404 when course not found', async () => {
    ...
    const req = new NextRequest('http://localhost/staff/courses/999')
