...
import { PUT } from '@/app/api/staff/admissions/aptitude/banks/[id]/questions/[questionId]/route.ts'
import { DELETE } from '@/app/api/staff/admissions/aptitude/banks/[id]/questions/[questionId]/route.ts'
import { getAuthSession, requireStaff, requireAdmin, requireAuth } from '@/lib/auth/helpers'

describe('PUT/DELETE /api/staff/admissions/aptitude/banks/[id]/questions/[questionId]', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  describe('PUT', () => {
    it('returns 401 when unauthenticated', async () => {
      ...
      const req = new NextRequest('http://localhost/api/staff/admissions/aptitude/banks/1/questions/1', {
        method: 'PUT',
        headers: { Authorization: 'Bearer test-secret' },
        body: JSON.stringify({}),
      })
      const res = await PUT(req, { params: Promise.resolve({ id: '1', questionId: '1' }) })
      expect(res.status).toBe(401)
    })

    it('returns 404 when resource not found', async () => {
      prismaMock.aptitudeQuestion.update.mockRejectedValueOnce({ code: 'P2025' } as any)
      const req = new NextRequest('http://localhost/api/staff/admissions/aptitude/banks/1/questions/1', {
        method: 'PUT',
        headers: { Authorization: 'Bearer test-secret' },
  ...
  describe('DELETE', () => {
    it('returns 401 when unauthenticated', async () => {
      ...
      const res = await DELETE(req, { params: Promise.resolve({ id: '1', questionId: '1' }) })
      expect(res.status).toBe(401)
    })

    it('returns 404 when resource not found', async () => {
      prismaMock.aptitudeQuestion.findUnique.mockResolvedValueOnce(null)
      const req = new NextRequest('http://localhost/api/staff/admissions/aptitude/banks/1/questions/1', {
        method: 'DELETE',
        headers: { Authorization: 'Bearer test-secret' },
      })
