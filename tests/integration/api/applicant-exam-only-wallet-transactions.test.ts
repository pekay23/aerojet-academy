vi.mock('@/lib/auth/helpers', () => ({
  ...
  requireApplicant: vi.fn().mockResolvedValue({ id: 'user-1', email: 'test@test.com', role: 'APPLICANT' }),
...
import { GET } from '@/app/api/applicant/exam-only/wallet/transactions/route.ts'
...
describe('/applicant/exam-only/wallet/transactions', () => {
  ...
  it('returns 200 with transactions and payments', async () => {
    prismaMock.walletTransaction.findMany.mockResolvedValue([
      { id: 'wt-1', amount: 100, type: 'TOP_UP', createdAt: new Date('2024-01-01'), referenceType: null, referenceId: null, description: null },
    ] as any)
    prismaMock.payment.findMany.mockResolvedValue([
      { id: 'p-1', amount: 100, status: 'COMPLETED', paymentMethod: null, createdAt: new Date('2024-01-01'), rejectionReason: null },
    ] as any)
