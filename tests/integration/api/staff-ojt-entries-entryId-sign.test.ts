...
import { POST } from '@/app/api/staff/ojt/entries/[entryId]/sign/route.ts'
import { getAuthSession, requireStaff, requireAdmin, requireAuth } from '@/lib/auth/helpers'

describe('POST /api/staff/ojt/entries/[entryId]/sign', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  describe('POST', () => {
    it('returns 401 when unauthenticated', async () => {
      ...
      const req = new NextRequest('http://localhost/api/staff/ojt/entries/entry-1/sign', {
        method: 'POST',
    ...
    it('signs entry successfully', async () => {
      prismaMock.oJTLogbookEntry.findUnique.mockResolvedValueOnce({
        id: 'entry-1',
