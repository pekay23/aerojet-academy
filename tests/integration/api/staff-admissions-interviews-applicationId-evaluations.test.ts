...
import { GET } from '@/app/api/staff/admissions/interviews/[applicationId]/evaluations/route.ts'
import { POST } from '@/app/api/staff/admissions/interviews/[applicationId]/evaluations/route.ts'
...
describe('GET/POST /api/staff/admissions/interviews/[applicationId]/evaluations', () => {
  ...
  describe('GET', () => {
      ...
      prismaMock.interviewEvaluation.findMany.mockResolvedValueOnce([
        {
          ...
          evaluator: { id: 'staff-1', email: 'test@test.com', profile: { firstName: 'Test', lastName: 'User' } },
        }
      ] as any)
      const req = new NextRequest('http://localhost/api/staff/admissions/interviews/app-1/evaluations?page=1&limit=20', {
    ...
    it('returns empty list when no evaluations', async () => {
      prismaMock.interviewEvaluation.findMany.mockResolvedValueOnce([])
      const req = new NextRequest('http://localhost/api/staff/admissions/interviews/app-1/evaluations', {
        headers: { Authorization: 'Bearer test-secret' },
      })
      const res = await GET(req, { params: Promise.resolve({ applicationId: 'app-1' }) })
      expect(res.status).toBe(200)
  ...
  describe('POST', () => {
    ...
    it('creates evaluation on valid input', async () => {
      prismaMock.application.findUnique.mockResolvedValueOnce({ id: 'app-1', stage: 'INTERVIEW_SCHEDULED' } as any)
