...
import { POST } from '@/app/api/staff/exams/internal/operations/publish/route.ts'
import { requireStaff } from '@/lib/auth/helpers'

describe('/staff/exams/internal/operations/publish', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('returns 401 when unauthenticated', async () => {
    ...
    const req = new NextRequest('http://localhost/staff/exams/internal/operations/publish', {
      method: 'POST',
      body: JSON.stringify({}),
    })
  ...
  it('returns 200 for valid publish', async () => {
    prismaMock.internalExamSession.findMany.mockResolvedValueOnce([{ id: 's1', studentId: 'stu1', bankId: 'b1', percentage: 80, passed: true }] as any)
    prismaMock.internalExamSession.updateMany.mockResolvedValueOnce({ count: 1 } as any)
    const req = new NextRequest('http://localhost/staff/exams/internal/operations/publish', {
      method: 'POST',
  ...
  it('returns 200 when no candidates to publish', async () => {
    prismaMock.internalExamSession.findMany.mockResolvedValueOnce([] as any)
    const req = new NextRequest('http://localhost/staff/exams/internal/operations/publish', {
      method: 'POST',
      body: JSON.stringify({ sessionIds: ['s1'] }),
    })
