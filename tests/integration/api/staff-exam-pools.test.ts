...
describe('/staff/exam-pools', () => {
  ...
  it('returns 401 when unauthenticated', async () => {
    ...
    const req = new NextRequest('http://localhost/staff/exam-pools', {
      method: 'POST',
      body: JSON.stringify({}),
    })
  ...
  it('returns 404 when event not found', async () => {
    ;(validateBody as any).mockReturnValueOnce({
      success: true,
      data: { eventId: 'evt-1', name: 'Pool A', examDate: '2025-01-01', examStartTime: '2025-01-01T09:00:00Z', examEndTime: '2025-01-01T12:00:00Z' }
    })
  ...
  it('returns 201 for valid creation', async () => {
    ...
    prismaMock.examEvent.findUnique.mockResolvedValueOnce({ id: 'evt-1' } as any)
    prismaMock.examPool.create.mockResolvedValueOnce({ id: 'pool-1' } as any)
    const req = new NextRequest('http://localhost/staff/exam-pools', {
      method: 'POST',
      body: JSON.stringify({ eventId: 'evt-1', name: 'Pool A' }),
    })
    const res = await POST(req)
