...
vi.mock('@/lib/analytics/events', () => ({ trackEnrollment: vi.fn().mockResolvedValue({ success: true }) }))

vi.mock('@/lib/security/rate-limit', () => ({
  rateLimitByUser: vi.fn().mockReturnValue({ allowed: true }),
}))

import { POST } from '@/app/api/staff/enrollments/batch/route.ts'
...
describe('/staff/enrollments/batch', () => {
  ...
  it('returns 403 when user is not staff role', async () => {
    ;(getAuthSession as any).mockResolvedValueOnce({ user: { id: 'user-1', email: 'test@test.com', role: 'STUDENT' } })
    const req = new NextRequest('http://localhost/api/staff/enrollments/batch', {
      method: 'POST',
      body: JSON.stringify({}),
    })
  ...
  it('returns 200 on successful batch enrollment', async () => {
    prismaMock.user.findMany.mockResolvedValueOnce([{ id: 's1' }] as any)
    prismaMock.course.findMany.mockResolvedValueOnce([{ id: 'c1', code: 'ATPL101', price: 500 }] as any)
