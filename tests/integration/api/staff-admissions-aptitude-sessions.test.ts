...
import { GET } from '@/app/api/staff/admissions/aptitude/sessions/route.ts'
import { getAuthSession, requireStaff, requireAdmin, requireAuth } from '@/lib/auth/helpers'

describe('GET /api/staff/admissions/aptitude/sessions', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  describe('GET', () => {
    it('returns list of sessions', async () => {
      prismaMock.aptitudeTestSession.findMany.mockResolvedValueOnce([{ id: '1' }] as any)
      const req = new NextRequest('http://localhost/api/staff/admissions/aptitude/sessions', {
    ...
    it('returns empty list when no data', async () => {
      prismaMock.aptitudeTestSession.findMany.mockResolvedValueOnce([])
      const req = new NextRequest('http://localhost/api/staff/admissions/aptitude/sessions', {
        headers: { Authorization: 'Bearer test-secret' },
      })
      const res = await GET(req)
