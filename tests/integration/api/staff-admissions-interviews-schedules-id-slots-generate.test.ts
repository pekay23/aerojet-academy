...
vi.mock('@/lib/settings', () => ({
  getInterviewConfig: vi.fn().mockResolvedValue({ success: true }),
}))

import { POST } from '@/app/api/staff/admissions/interviews/schedules/[id]/slots/generate/route.ts'
import { getAuthSession, requireStaff, requireAdmin, requireAuth } from '@/lib/auth/helpers'

describe('POST /api/staff/admissions/interviews/schedules/[id]/slots/generate', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  describe('POST', () => {
    it('returns 401 when unauthenticated', async () => {
      ...
      const req = new NextRequest('http://localhost/api/staff/admissions/interviews/schedules/schedule-1/slots/generate', {
        method: 'POST',
    ...
    it('creates resource on valid input', async () => {
      ...
      prismaMock.interviewSlot.createMany.mockResolvedValueOnce({ count: 1 } as any)
      const req = new NextRequest('http://localhost/api/staff/admissions/interviews/schedules/schedule-1/slots/generate', {
        method: 'POST',
        headers: { Authorization: 'Bearer test-secret' },
        body: JSON.stringify({
          dateStart: '2024-01-01T00:00:00.000Z',
          dateEnd: '2024-01-07T23:59:00.000Z',
          daysOfWeek: [0, 1, 2, 3, 4, 5, 6],
          timeBlocks: [{ start: '09:00', end: '10:00', capacity: 1, location: 'Room A' }],
        }),
      })
      const res = await POST(req, params)
