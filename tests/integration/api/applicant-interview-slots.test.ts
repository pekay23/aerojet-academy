...
import { GET } from '@/app/api/applicant/interview/slots/route.ts'
import { getAuthSession } from '@/lib/auth/helpers'

describe('/applicant/interview/slots', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    ;(getAuthSession as any).mockResolvedValue({ user: { id: 'user-1', email: 'test@test.com', role: 'APPLICANT' } })
    prismaMock.application.findUnique.mockResolvedValue({ id: 'app-1', userId: 'user-1', stage: 'INTERVIEW_PENDING', intakeCycleId: 'cycle-1', interviewSlotId: null } as any)
    prismaMock.interviewSchedule.findMany.mockResolvedValue([
      {
        id: 'schedule-1',
        slots: [
          { id: 'slot-1', date: new Date(Date.now() + 86400000), startTime: new Date(Date.now() + 86400000), capacity: 10, bookedCount: 5 },
        ],
      },
    ] as any)
  })
  ...
  it('returns 401 when unauthenticated', async () => {
    ...
    const req = new NextRequest('http://localhost/applicant/interview/slots')
    const res = await GET(req)
    expect(res.status).toBe(401)
  })

  it('returns 200 with data', async () => {
    const req = new NextRequest('http://localhost/applicant/interview/slots?page=1&limit=20')
    const res = await GET(req)
