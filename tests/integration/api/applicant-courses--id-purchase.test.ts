...
vi.mock('@/lib/analytics/events', () => ({ trackEnrollment: vi.fn().mockResolvedValue({ success: true }), trackPaymentSubmitted: vi.fn().mockResolvedValue({ success: true }) }))

import { describe, it, expect, vi, beforeEach } from 'vitest'
import { prismaMock } from '@/tests/setup'
import { NextRequest } from 'next/server'
import { POST } from '@/app/api/applicant/courses/[id]/purchase/route.ts'
import { requireApplicant } from '@/lib/auth/helpers'

const params = { params: Promise.resolve({ id: '1' }) }

describe('/applicant/courses/:id/purchase', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    ;(requireApplicant as any).mockResolvedValue({ id: 'user-1', role: 'APPLICANT' })
    prismaMock.course.findUnique.mockResolvedValue({ id: '1', code: 'ABC', name: 'Test Course', price: 100, currency: 'EUR', isActive: true } as any)
    prismaMock.enrollment.findFirst.mockResolvedValue(null as any)
    prismaMock.enrollment.create.mockResolvedValue({ id: 'enroll-1', userId: 'user-1', courseId: '1', status: 'PENDING' } as any)
    prismaMock.payment.create.mockResolvedValue({ id: 'pay-1', userId: 'user-1', amount: 100, currency: 'EUR', status: 'PENDING' } as any)
  })

  it('returns 401 when unauthenticated', async () => {
    ;(requireApplicant as any).mockRejectedValueOnce(new Error('Unauthorized'))
    const req = new NextRequest('http://localhost/applicant/courses/1/purchase', {
      method: 'POST',
      body: JSON.stringify({}),
    })
  ...
  it('returns 400 when already enrolled', async () => {
    prismaMock.enrollment.findFirst.mockResolvedValueOnce({ id: 'existing' } as any)
    const req = new NextRequest('http://localhost/applicant/courses/1/purchase', {
      method: 'POST',
      body: JSON.stringify({}),
    })
