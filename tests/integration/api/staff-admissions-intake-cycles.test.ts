...
import { GET } from '@/app/api/staff/admissions/intake-cycles/route.ts'
import { POST } from '@/app/api/staff/admissions/intake-cycles/route.ts'
import { getAuthSession, requireStaff, requireAdmin, requireAuth } from '@/lib/auth/helpers'

describe('GET/POST /api/staff/admissions/intake-cycles', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  describe('GET', () => {
    it('returns paginated list', async () => {
      prismaMock.intakeCycle.findMany.mockResolvedValueOnce([{ id: '1' }] as any)
      prismaMock.intakeCycle.count.mockResolvedValueOnce(1)
      const req = new NextRequest('http://localhost/api/staff/admissions/intake-cycles?page=1&limit=20', {
    ...
    it('returns empty list when no data', async () => {
      prismaMock.intakeCycle.findMany.mockResolvedValueOnce([])
      prismaMock.intakeCycle.count.mockResolvedValueOnce(0)
      const req = new NextRequest('http://localhost/api/staff/admissions/intake-cycles', {
        headers: { Authorization: 'Bearer test-secret' },
      })
      const res = await GET(req)
      expect(res.status).toBe(200)
  ...
  describe('POST', () => {
    ...
    it('creates resource on valid input', async () => {
      prismaMock.intakeCycle.create.mockResolvedValueOnce({ id: '1' } as any)
      const req = new NextRequest('http://localhost/api/staff/admissions/intake-cycles', {
        method: 'POST',
        headers: { Authorization: 'Bearer test-secret' },
        body: JSON.stringify({ name: 'test', startDate: '2024-01-01', endDate: '2024-12-31' }),
      })
      const res = await POST(req)
