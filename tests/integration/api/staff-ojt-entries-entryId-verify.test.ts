...
import { POST } from '@/app/api/staff/ojt/entries/[entryId]/verify/route.ts'
import { getAuthSession, requireStaff, requireAdmin, requireAuth } from '@/lib/auth/helpers'

describe('POST /api/staff/ojt/entries/[entryId]/verify', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  describe('POST', () => {
    it('returns 401 when unauthenticated', async () => {
      ...
      const req = new NextRequest('http://localhost/api/staff/ojt/entries/entry-1/verify', {
        method: 'POST',
    ...
    it('verifies entry successfully', async () => {
      prismaMock.oJTLogbookEntry.findUnique.mockResolvedValueOnce({
        id: 'entry-1',
