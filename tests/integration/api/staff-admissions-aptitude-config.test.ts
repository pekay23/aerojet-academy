...
vi.mock('@/lib/settings', () => ({
  getAptitudeConfig: vi.fn().mockReturnValue({ success: true }),
  updateSystemSetting: vi.fn().mockReturnValue({ success: true })
}))

import { GET } from '@/app/api/staff/admissions/aptitude/config/route.ts'
import { PUT } from '@/app/api/staff/admissions/aptitude/config/route.ts'
import { getAuthSession, requireStaff, requireAdmin, requireAuth } from '@/lib/auth/helpers'

describe('GET/PUT /api/staff/admissions/aptitude/config', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  describe('GET', () => {
    it('returns config', async () => {
      const req = new NextRequest('http://localhost/api/staff/admissions/aptitude/config', {
  ...
  describe('PUT', () => {
    ...
    it('updates config on valid input', async () => {
      const req = new NextRequest('http://localhost/api/staff/admissions/aptitude/config', {
        method: 'PUT',
        headers: { Authorization: 'Bearer test-secret' },
        body: JSON.stringify({ aptitude_time_limit_minutes: '30' }),
      })
      const res = await PUT(req)
      expect(res.status).toBe(200)
    })
