...
import { GET } from '@/app/api/staff/instructors/[id]/route.ts'
import { PUT } from '@/app/api/staff/instructors/[id]/route.ts'
import { getAuthSession, requireStaff, requireAdmin, requireAuth } from '@/lib/auth/helpers'

describe('GET/PUT /api/staff/instructors/[id]', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  describe('GET', () => {
    it('returns 401 when unauthenticated', async () => {
      ...
      const req = new NextRequest('http://localhost/api/staff/instructors/1', {
    ...
    it('returns instructor detail by id', async () => {
      prismaMock.instructorProfile.findUnique.mockResolvedValueOnce({
        id: '1',
        userId: 'user-1',
        employeeId: 'EMP-001',
        department: 'Flight Training',
        specialization: 'Fixed Wing',
        modulesQualified: ['MOD-1'],
    ...
    it('returns 404 when instructor not found', async () => {
      ...
      const req = new NextRequest('http://localhost/api/staff/instructors/999', {
        headers: { Authorization: 'Bearer test-secret' },
      })
  ...
  describe('PUT', () => {
    ...
    it('updates instructor profile', async () => {
      prismaMock.instructorProfile.update.mockResolvedValueOnce({
        id: '1',
        department: 'Ground School',
        specialization: 'Theory',
        modulesQualified: ['MOD-1', 'MOD-2'],
      } as any)
      const req = new NextRequest('http://localhost/api/staff/instructors/1', {
        method: 'PUT',
        headers: { Authorization: 'Bearer test-secret' },
        body: JSON.stringify({ department: 'Ground School', specialization: 'Theory', modulesQualified: ['MOD-1', 'MOD-2'] }),
      })
      const res = await PUT(req, { params: Promise.resolve({ id: '1' }) })
      expect(res.status).toBe(200)
      const json = await res.json()
