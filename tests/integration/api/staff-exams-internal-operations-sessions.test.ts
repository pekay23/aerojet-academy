...
import { GET } from '@/app/api/staff/exams/internal/operations/sessions/route.ts'
import { requireStaff } from '@/lib/auth/helpers'

describe('/staff/exams/internal/operations/sessions', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('returns 401 when unauthenticated', async () => {
    ...
    const req = new NextRequest('http://localhost/staff/exams/internal/operations/sessions')
    const res = await GET(req)
    expect(res.status).toBe(401)
  })

  it('returns 400 for invalid query', async () => {
    const req = new NextRequest('http://localhost/staff/exams/internal/operations/sessions?status=INVALID')
    const res = await GET(req)
