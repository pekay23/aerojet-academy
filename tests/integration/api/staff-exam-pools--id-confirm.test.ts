...
vi.mock('@/lib/pools/operations', () => ({
  confirmPool: vi.fn().mockResolvedValue({ id: 'pool-1' }),
}))
...
import { POST } from '@/app/api/staff/exam-pools/[id]/confirm/route.ts'
import { requireAdmin } from '@/lib/auth/helpers'

describe('/staff/exam-pools/:id/confirm', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    ;(requireAdmin as any).mockResolvedValue({ id: 'user-1', role: 'ADMIN' })
  })

  it('returns 401 when unauthenticated', async () => {
    ;(requireAdmin as any).mockRejectedValueOnce(new Error('Unauthorized'))
    const req = new NextRequest('http://localhost/staff/exam-pools/1/confirm', {
      method: 'POST',
      body: JSON.stringify({}),
    })
  ...
  it('returns 400 for missing pool id', async () => {
    const req = new NextRequest('http://localhost/staff/exam-pools/1/confirm', {
      method: 'POST',
      body: JSON.stringify({}),
    })
