...
import { GET } from '@/app/api/staff/documents/expiring/route.ts'
import { getAuthSession, requireStaff, requireAdmin, requireAuth } from '@/lib/auth/helpers'

describe('GET /api/staff/documents/expiring', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  describe('GET', () => {
    it('returns expiring documents and licenses', async () => {
      prismaMock.studentDocument.findMany.mockResolvedValueOnce([])
      prismaMock.studentLicenseTarget.findMany.mockResolvedValueOnce([])
      const req = new NextRequest('http://localhost/api/staff/documents/expiring', {
    ...
    it('returns documents within a custom window', async () => {
      prismaMock.studentDocument.findMany.mockResolvedValueOnce([])
      prismaMock.studentLicenseTarget.findMany.mockResolvedValueOnce([])
      const req = new NextRequest('http://localhost/api/staff/documents/expiring?window=30', {
        headers: { Authorization: 'Bearer test-secret' },
      })
      const res = await GET(req)
      expect(res.status).toBe(200)
