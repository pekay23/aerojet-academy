...
vi.mock('@/lib/admissions/state-machine', () => ({ transitionApplication: vi.fn().mockResolvedValue({ success: true }) }))
vi.mock('@/lib/settings', () => ({ getInterviewConfig: vi.fn().mockResolvedValue({ interview_max_reschedules: 2, interview_reschedule_cutoff_hours: 24 }) }))

import { describe, it, expect, vi, beforeEach } from 'vitest'
import { prismaMock } from '@/tests/setup'
import { NextRequest } from 'next/server'
import { POST } from '@/app/api/applicant/interview/book/route.ts'
import { getAuthSession } from '@/lib/auth/helpers'

describe('/applicant/interview/book', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    ;(getAuthSession as any).mockResolvedValue({ user: { id: 'user-1', email: 'test@test.com', role: 'APPLICANT' } })
    prismaMock.notification.create.mockResolvedValue({} as any)
    prismaMock.application.findUnique.mockResolvedValue({ id: 'app-1', userId: 'user-1', stage: 'INTERVIEW_PENDING', interviewSlotId: null, interviewRescheduleCount: 0 } as any)
    prismaMock.interviewSlot.findUnique.mockResolvedValue({ id: 'slot-1', startTime: new Date(Date.now() + 86400000), capacity: 10, bookedCount: 5 } as any)
    prismaMock.interviewSlot.update.mockResolvedValue({ id: 'slot-1', bookedCount: 5 } as any)
    prismaMock.interviewSlot.updateMany.mockResolvedValue({ count: 1 } as any)
    prismaMock.application.update.mockResolvedValue({ id: 'app-1', interviewSlotId: 'slot-1' } as any)
  })

  it('returns 401 when unauthenticated', async () => {
    ;(getAuthSession as any).mockRejectedValueOnce(new Error('Unauthorized'))
    const req = new NextRequest('http://localhost/applicant/interview/book', {
      method: 'POST',
  ...
  it('returns 200 on successful booking', async () => {
    const req = new NextRequest('http://localhost/applicant/interview/book', {
      method: 'POST',
      body: JSON.stringify({ slotId: 'slot-1' }),
    })
