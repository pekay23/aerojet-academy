...
vi.mock('@/lib/internal-exam/engine', () => ({
  getBankRules: vi.fn().mockResolvedValue({ passMarkPct: 75 }),
}))

import { POST } from '@/app/api/staff/exams/internal/operations/regrade/route.ts'
import { requireStaff } from '@/lib/auth/helpers'

describe('/staff/exams/internal/operations/regrade', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('returns 401 when unauthenticated', async () => {
    ...
    const req = new NextRequest('http://localhost/staff/exams/internal/operations/regrade', {
      method: 'POST',
      body: JSON.stringify({}),
    })
  ...
  it('returns 200 for valid regrade', async () => {
    prismaMock.internalExamSession.findUnique.mockResolvedValueOnce({
      ...
      answers: [{
        ...
        isCorrect: false,
        pointsAwarded: 0,
        question: { id: 'q1', correctAnswer: 'A', points: 1 }
      }]
    } as any)
    prismaMock.internalExamAnswer.update.mockResolvedValueOnce({} as any)
    prismaMock.internalExamSession.update.mockResolvedValueOnce({} as any)
    const req = new NextRequest('http://localhost/staff/exams/internal/operations/regrade', {
      method: 'POST',
