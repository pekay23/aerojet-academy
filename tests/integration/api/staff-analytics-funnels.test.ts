...
describe('GET /api/staff/analytics/funnels', () => {
  ...
  it('passes from/to params to query', async () => {
    ...
    const req = new Request('http://localhost/api/staff/analytics/funnels?funnel=enrollment&from=2024-01-01&to=2024-12-31')
    const res = await GET(req as any)
    expect(res.status).toBe(200)
    expect(getFunnelMetrics).toHaveBeenCalledWith('enrollment', new Date('2024-01-01'), new Date('2024-12-31'))
  })
})
