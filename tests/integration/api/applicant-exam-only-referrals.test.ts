...
vi.mock('@/lib/referral/operations', () => ({
  getOrCreateReferralCode: vi.fn().mockResolvedValue('AERO-ABC123'),
  getReferralStats: vi.fn().mockResolvedValue({
    referralCode: 'AERO-ABC123',
    isAmbassador: false,
    totalReferrals: 0,
...
import { GET, POST } from '@/app/api/applicant/exam-only/referrals/route.ts'
import { requireApplicant } from '@/lib/auth/helpers'

describe('/applicant/exam-only/referrals', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('returns 401 when unauthenticated on GET', async () => {
    ...
    const req = new NextRequest('http://localhost/applicant/exam-only/referrals')
  ...
  it('returns 200 with referral stats on GET', async () => {
    const req = new NextRequest('http://localhost/applicant/exam-only/referrals')
    const res = await GET(req)
    expect(res.status).toBe(200)
    const json = await res.json()
    expect(json).toBeDefined()
    expect(json.referralCode).toBe('AERO-ABC123')
  })
  ...
  it('returns 200 with referral code on POST', async () => {
    const req = new NextRequest('http://localhost/applicant/exam-only/referrals', {
      method: 'POST',
      body: JSON.stringify({}),
    })
    ...
    expect(res.status).toBe(200)
    const json = await res.json()
    expect(json).toEqual({ referralCode: 'AERO-ABC123' })
  })
})
