...
import { POST } from '@/app/api/applicant/aptitude/start/route.ts'
...
describe('/applicant/aptitude/start', () => {
  beforeEach(() => {
    ...
    prismaMock.aptitudeTestSession.create.mockResolvedValue({ id: 'session-1', expiresAt: new Date() } as any)
  ...
  it('returns 403 when application stage is not APTITUDE_PENDING', async () => {
    prismaMock.application.findUnique.mockResolvedValueOnce({ id: 'app-1', stage: 'UNDER_REVIEW' } as any)
    const req = new NextRequest('http://localhost/applicant/aptitude/start', {
      method: 'POST',
      body: JSON.stringify({}),
    })
  ...
  it('returns 500 when no active test banks found', async () => {
    prismaMock.application.findUnique.mockResolvedValueOnce({ id: 'app-1', stage: 'APTITUDE_PENDING' } as any)
  ...
  it('returns 200 with sessionId and expiresAt on success', async () => {
    prismaMock.application.findUnique.mockResolvedValueOnce({ id: 'app-1', stage: 'APTITUDE_PENDING', programmeChoice: 'PLANE_MAINTENANCE' } as any)
    prismaMock.aptitudeTestBank.findMany.mockResolvedValueOnce([{ id: 'bank-1', applicableProgrammes: ['PLANE_MAINTENANCE'], isActive: true }] as any)
    prismaMock.aptitudeTestSession.create.mockResolvedValueOnce({ id: 'session-1', expiresAt: new Date('2026-12-31T23:59:59Z') } as any)
    const req = new NextRequest('http://localhost/applicant/aptitude/start', {
      method: 'POST',
      body: JSON.stringify({}),
    })
