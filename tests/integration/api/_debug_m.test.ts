  ...
  withErrorHandler: vi.fn((fn) => {
    return async (req, ctx) => {
      try {
        ...
        console.log('CAUGHT ERROR:', (err as Error)?.stack || String(err))
...
import { POST } from '@/app/api/staff/admissions/medical/review/route.ts'
...
describe('POST /api/staff/admissions/medical/review', () => {
  ...
  it('creates resource on valid input', async () => {
    ...
    const req = new NextRequest('http://localhost/api/staff/admissions/medical/review', {
      ...
      body: JSON.stringify({ applicationId: 'app-1', decision: 'CLEARED', notes: 'ok', facility: 'clinic' }),
    })
