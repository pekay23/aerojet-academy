...
import { GET, POST } from '@/app/api/staff/exam-events/[id]/pools/route.ts'
import { requireStaff } from '@/lib/auth/helpers'
import { validateBody } from '@/lib/validation/schemas'

describe('/staff/exam-events/:id/pools', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('returns 401 when unauthenticated (GET)', async () => {
    ...
    const req = new NextRequest('http://localhost/staff/exam-events/1/pools')
    const res = await GET(req, { params: { id: '1' } } as any)
    expect(res.status).toBe(401)
  })

  it('returns 200 with data', async () => {
    prismaMock.examPool.findMany.mockResolvedValue([{ id: '1', _count: { memberships: 0 } }] as any)
    const req = new NextRequest('http://localhost/staff/exam-events/1/pools')
    const res = await GET(req, { params: { id: '1' } } as any)
  ...
  it('returns 201 for valid creation', async () => {
    ;(validateBody as any).mockReturnValueOnce({
      success: true,
      data: { name: 'Pool A', examDate: '2025-01-01', examStartTime: '2025-01-01T09:00:00Z', examEndTime: '2025-01-01T12:00:00Z' }
    })
