...
import { POST } from '@/app/api/applicant/aptitude/anti-cheat/route.ts'
...
describe('/applicant/aptitude/anti-cheat', () => {
  ...
  it('returns 403 when session belongs to another user', async () => {
    prismaMock.aptitudeTestSession.findUnique.mockResolvedValueOnce({ id: 'ck1a1b2c3d0001aaaa1111a', userId: 'user-2', status: 'IN_PROGRESS', tabSwitchCount: 0, fullscreenExits: 0 } as any)
    const req = new NextRequest('http://localhost/applicant/aptitude/anti-cheat', {
      method: 'POST',
      body: JSON.stringify({ sessionId: 'ck1a1b2c3d0001aaaa1111a', eventType: 'TAB_SWITCH' }),
    })
  ...
  it('returns 200 with ignored when session not in progress', async () => {
    prismaMock.aptitudeTestSession.findUnique.mockResolvedValueOnce({ id: 'ck1a1b2c3d0001aaaa1111a', userId: 'user-1', status: 'COMPLETED', tabSwitchCount: 0, fullscreenExits: 0 } as any)
    const req = new NextRequest('http://localhost/applicant/aptitude/anti-cheat', {
      method: 'POST',
      body: JSON.stringify({ sessionId: 'ck1a1b2c3d0001aaaa1111a', eventType: 'TAB_SWITCH' }),
    })
  ...
  it('returns 200 with FLAGGED when violations exceed limit', async () => {
    prismaMock.aptitudeTestSession.findUnique.mockResolvedValueOnce({ id: 'ck1a1b2c3d0001aaaa1111a', userId: 'user-1', status: 'IN_PROGRESS', tabSwitchCount: 3, fullscreenExits: 0 } as any)
    const req = new NextRequest('http://localhost/applicant/aptitude/anti-cheat', {
      method: 'POST',
