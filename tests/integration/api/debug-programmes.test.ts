  ...
  withErrorHandler: vi.fn((fn) => {
    return async (req: any, ctx: any) => {
      try {
        ...
        return { status: 500, json: () => Promise.resolve({ error: 'Internal Server Error', details: message }) }
      }
    }
  }),
}))

import { GET } from '@/app/api/staff/programmes/route.ts'

describe('debug programmes', () => {
  it('debug GET', async () => {
    console.log('keys', Object.keys(prismaMock))
    console.log('fullTimeProgramme', (prismaMock as any).fullTimeProgramme)
