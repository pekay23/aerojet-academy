...
vi.mock('@/lib/analytics/events', () => ({ trackPaymentSubmitted: vi.fn().mockResolvedValue({ success: true }) }))

import { describe, it, expect, vi, beforeEach } from 'vitest'
import { prismaMock } from '@/tests/setup'
import { NextRequest } from 'next/server'
import { POST } from '@/app/api/applicant/exam-only/top-up/route.ts'
...
describe('/applicant/exam-only/top-up', () => {
  beforeEach(() => {
    ...
    prismaMock.user.findUnique.mockResolvedValue({ registrationCode: 'REG-123', programmeChoice: 'ATPL' } as any)
    prismaMock.payment.findFirst.mockResolvedValue(null as any)
    prismaMock.payment.create.mockResolvedValue({ id: 'pay-1', amount: 500, currency: 'EUR', userId: 'user-1', paymentMethod: 'BANK_TRANSFER' } as any)
  })
  ...
  it('returns 400 when no registration code is found', async () => {
    prismaMock.user.findUnique.mockResolvedValueOnce({ registrationCode: null, programmeChoice: null } as any)
    const req = new NextRequest('http://localhost/applicant/exam-only/top-up', {
      method: 'POST',
