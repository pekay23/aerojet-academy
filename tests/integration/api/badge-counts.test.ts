...
vi.mock('@/app/api/badge-counts/cache', () => ({
  BadgeCountsCache: class {
    ...
    getGlobal(key: string) { return this.globalCache.get(key)?.data ?? null }
    setGlobal(key: string, data: any) { this.globalCache.set(key, { data, timestamp: new Date() }) }
    ...
    invalidateGlobal() { this.globalCache.clear() }
  }
}))
