vi.mock('@/lib/auth/helpers', () => ({
 ...
 vi.mock('@/lib/validation/schemas', () => ({ changePasswordSchema: vi.fn(), validateBody: vi.fn().mockReturnValue({ success: true, data: { currentPassword: 'old', newPassword: 'new' } }) }))
vi.mock('@/lib/audit/logger', () => ({
  createAuditLog: vi.fn(),
  logAuditEvent: vi.fn(),
  AuditAction: { PASSWORD_CHANGE: 'PASSWORD_CHANGE', CREATE: 'CREATE', UPDATE: 'UPDATE', DELETE: 'DELETE', PAYMENT_APPROVE: 'PAYMENT_APPROVE', ENROLLMENT_APPROVE: 'ENROLLMENT_APPROVE' },
}))
vi.mock('@/lib/api/response', () => ({
 ...
 import { POST } from '@/app/api/instructor/profile/change-password/route.ts'
