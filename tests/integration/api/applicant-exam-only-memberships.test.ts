...
import { GET } from '@/app/api/applicant/exam-only/memberships/route.ts'
import { requireApplicant } from '@/lib/auth/helpers'

describe('/applicant/exam-only/memberships', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('returns 401 when unauthenticated', async () => {
    ...
    const req = new NextRequest('http://localhost/applicant/exam-only/memberships')
  ...
  it('returns 200 with data', async () => {
    prismaMock.poolMembership.findMany.mockResolvedValue([
      {
        ...
        pool: {
          ...
          examStartTime: new Date('2025-01-01T09:00:00Z'),
          examEndTime: new Date('2025-01-01T12:00:00Z'),
          status: 'ACTIVE',
    ...
    const req = new NextRequest('http://localhost/applicant/exam-only/memberships?page=1&limit=20')
    const res = await GET(req)
    ...
    expect(json.memberships[0].id).toBe('1')
    expect(json.memberships[0].pool.name).toBe('Test Pool')
    expect(json.memberships[0].examComponent.name).toBe('Written')
    expect(json.memberships[0].examComponent.course.code).toBe('ATPL')
  })
})
