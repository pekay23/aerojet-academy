...
import { POST } from '@/app/api/staff/admissions/interviews/outcome/route.ts'
import { getAuthSession, requireStaff, requireAdmin, requireAuth } from '@/lib/auth/helpers'

describe('POST /api/staff/admissions/interviews/outcome', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  describe('POST', () => {
    it('returns 401 when unauthenticated', async () => {
      ...
      const req = new NextRequest('http://localhost/api/staff/admissions/interviews/outcome', {
        method: 'POST',
    ...
    it('records outcome for valid input', async () => {
      ...
      prismaMock.application.update.mockResolvedValueOnce({ id: 'app-1' } as any)
      const req = new NextRequest('http://localhost/api/staff/admissions/interviews/outcome', {
        method: 'POST',
        headers: { Authorization: 'Bearer test-secret' },
        body: JSON.stringify({ applicationId: 'app-1', outcome: 'PASS', score: 85, notes: 'Great' }),
      })
      const res = await POST(req)
