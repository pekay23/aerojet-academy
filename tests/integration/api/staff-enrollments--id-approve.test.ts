...
import { POST } from '@/app/api/staff/enrollments/[id]/approve/route.ts'
...
describe('/staff/enrollments/:id/approve', () => {
  ...
  it('returns 400 when enrollment is not PENDING', async () => {
    prismaMock.enrollment.findUnique.mockResolvedValueOnce({
      id: 'e1',
      status: 'ENROLLED',
      user: { id: 'u1', role: 'STUDENT', profile: { firstName: 'John', lastName: 'Doe' }, studentProfile: null },
  ...
  it('returns 200 on successful approval', async () => {
    prismaMock.enrollment.findUnique.mockResolvedValueOnce({
      id: 'e1',
      status: 'PENDING',
      user: { id: 'u1', role: 'STUDENT', profile: { firstName: 'John', lastName: 'Doe' }, studentProfile: { id: 'sp1' } },
      course: { id: 'c1', code: 'ATPL101' },
    } as any)
