...
describe('Applicant Exams — GET /api/applicant/exam-only/pools', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    requireApplicant.mockResolvedValue(mockUser)
  })

  it('returns 401 when unauthenticated', async () => {
    requireApplicant.mockRejectedValueOnce(new Error('Unauthorized'))
...
describe('Applicant Exams — GET /api/applicant/exam-only/bookings', () => {
  ...
  it('returns 401 when unauthenticated', async () => {
    requireApplicant.mockRejectedValueOnce(new Error('Unauthorized'))
    const req = new NextRequest('http://localhost/api/applicant/exam-only/bookings', { method: 'GET' })
  ...
  it('returns paginated bookings with correct shape', async () => {
    ...
    const req = new NextRequest('http://localhost/api/applicant/exam-only/bookings?page=1&limit=20', { method: 'GET' })
    const res = await GetBookings(req)
    expect(res.status).toBe(200)
