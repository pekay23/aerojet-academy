...
vi.mock('@/lib/aptitude/grading', () => ({
  gradeAptitudeTest: vi.fn().mockResolvedValue({ score: 10, passed: true }),
}))

import { describe, it, expect, vi, beforeEach } from 'vitest'
import { prismaMock } from '@/tests/setup'
import { NextRequest } from 'next/server'
import { POST } from '@/app/api/applicant/aptitude/submit/route.ts'
import { requireAuth } from '@/lib/auth/helpers'

describe('/applicant/aptitude/submit', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('returns 401 when unauthenticated', async () => {
    ...
    const req = new NextRequest('http://localhost/applicant/aptitude/submit', {
      method: 'POST',
      body: JSON.stringify({}),
    })
  ...
  it('returns 404 when session not found', async () => {
    ...
    const req = new NextRequest('http://localhost/applicant/aptitude/submit', {
      method: 'POST',
      body: JSON.stringify({ sessionId: 'ck1a1b2c3d0001aaaa1111a' }),
    })
    const res = await POST(req)
  ...
  it('returns 200 with message when session already processed', async () => {
    prismaMock.aptitudeTestSession.findUnique.mockResolvedValueOnce({ id: 'ck1a1b2c3d0001aaaa1111a', userId: 'user-1', status: 'COMPLETED' } as any)
    const req = new NextRequest('http://localhost/applicant/aptitude/submit', {
      method: 'POST',
      body: JSON.stringify({ sessionId: 'ck1a1b2c3d0001aaaa1111a' }),
    })
    ...
    expect(json).toEqual({ message: 'Session already processed' })
  })

  it('returns 200 with submitted, score, and passed on success', async () => {
    ...
    expect(res.status).toBe(200)
    const json = await res.json()
    expect(json).toEqual({ submitted: true, score: 10, passed: true })
  })
})
