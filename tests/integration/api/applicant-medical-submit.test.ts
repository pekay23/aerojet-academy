...
import { POST } from '@/app/api/applicant/medical/submit/route.ts'
import { requireApplicant } from '@/lib/auth/helpers'
import { transitionApplication } from '@/lib/admissions/state-machine'

describe('/applicant/medical/submit', () => {
  beforeEach(() => {
    vi.resetAllMocks()
    ;(requireApplicant as any).mockResolvedValue({ id: 'user-1', role: 'APPLICANT' })
    ;(transitionApplication as any).mockResolvedValue({ success: true })
    prismaMock.notification.create.mockResolvedValue({} as any)
    prismaMock.application.findUnique.mockResolvedValue({ id: 'app-1', userId: 'user-1', stage: 'MEDICAL_PENDING' } as any)
    prismaMock.applicationDocument.findMany.mockResolvedValue([{ id: 'doc-1', applicationId: 'app-1' }] as any)
    prismaMock.application.update.mockResolvedValue({ id: 'app-1', medicalStatus: 'DOCUMENTS_SUBMITTED' } as any)
  })

  it('returns 401 when unauthenticated', async () => {
    ;(requireApplicant as any).mockRejectedValueOnce(new Error('Unauthorized'))
    const req = new NextRequest('http://localhost/applicant/medical/submit', {
      method: 'POST',
      body: JSON.stringify({}),
    })
  ...
  it('returns 400 when no medical docs uploaded', async () => {
    prismaMock.application.findUnique.mockResolvedValueOnce({ id: 'app-1', userId: 'user-1', stage: 'MEDICAL_PENDING' } as any)
    prismaMock.applicationDocument.findMany.mockResolvedValueOnce([] as any)
    const req = new NextRequest('http://localhost/applicant/medical/submit', {
      method: 'POST',
      body: JSON.stringify({}),
    })
