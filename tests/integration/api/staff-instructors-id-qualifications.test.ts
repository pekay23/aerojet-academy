...
import { POST } from '@/app/api/staff/instructors/[id]/qualifications/route.ts'
import { getAuthSession, requireStaff, requireAdmin, requireAuth } from '@/lib/auth/helpers'

describe('POST /api/staff/instructors/[id]/qualifications', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  describe('POST', () => {
    it('returns 401 when unauthenticated', async () => {
      ...
      const req = new NextRequest('http://localhost/api/staff/instructors/1/qualifications', {
        method: 'POST',
    ...
    it('creates resource on valid input', async () => {
      prismaMock.instructorQualification.create.mockResolvedValueOnce({
        id: '1',
        instructorId: '1',
        qualificationType: 'CPL',
        issuedBy: 'CAA',
      ...
      const req = new NextRequest('http://localhost/api/staff/instructors/1/qualifications', {
        method: 'POST',
        headers: { Authorization: 'Bearer test-secret' },
        body: JSON.stringify({ qualificationType: 'CPL', issuedBy: 'CAA', issueDate: '2024-01-01', expiryDate: '2025-01-01', notes: 'test' }),
      })
      const res = await POST(req, { params: Promise.resolve({ id: '1' }) })
