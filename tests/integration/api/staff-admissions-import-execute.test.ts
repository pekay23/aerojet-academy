...
import { POST } from '@/app/api/staff/admissions/import/execute/route.ts'
import { getAuthSession, requireStaff, requireAdmin, requireAuth } from '@/lib/auth/helpers'

describe('POST /api/staff/admissions/import/execute', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  describe('POST', () => {
    it('returns 401 when unauthenticated', async () => {
      ...
      const req = new NextRequest('http://localhost/api/staff/admissions/import/execute', {
        method: 'POST',
    ...
    it('imports valid rows', async () => {
      ...
      prismaMock.application.create.mockResolvedValueOnce({ id: 'app-1' })
      const req = new NextRequest('http://localhost/api/staff/admissions/import/execute', {
        method: 'POST',
        headers: { Authorization: 'Bearer test-secret' },
        body: JSON.stringify({
