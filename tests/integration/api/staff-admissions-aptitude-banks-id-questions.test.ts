...
import { GET } from '@/app/api/staff/admissions/aptitude/banks/[id]/questions/route.ts'
import { POST } from '@/app/api/staff/admissions/aptitude/banks/[id]/questions/route.ts'
import { getAuthSession, requireStaff, requireAdmin, requireAuth } from '@/lib/auth/helpers'

describe('GET/POST /api/staff/admissions/aptitude/banks/[id]/questions', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  describe('GET', () => {
    it('returns list of questions', async () => {
      prismaMock.aptitudeQuestion.findMany.mockResolvedValueOnce([{ id: '1' }] as any)
      const req = new NextRequest('http://localhost/api/staff/admissions/aptitude/banks/1/questions', {
  ...
  describe('POST', () => {
    ...
    it('creates resource on valid input', async () => {
      ...
      prismaMock.aptitudeQuestion.create.mockResolvedValueOnce({ id: '1' } as any)
      const req = new NextRequest('http://localhost/api/staff/admissions/aptitude/banks/1/questions', {
        method: 'POST',
        headers: { Authorization: 'Bearer test-secret' },
        body: '{"category":"MATH","questionType":"MCQ","difficulty":"EASY","text":"test","correctAnswer":"test","points":1,"explanation":"test","isActive":true}',
      })
      const res = await POST(req, { params: Promise.resolve({ id: '1' }) })
