...
const path = 'app/student/courses/[slug]/materials/page.tsx';
...
// Replace href redirects from id to slug
content = content.replace(/redirect\(\`\/student\/courses\/\$\{id\}\?error=payment_required\`\)/g, 'redirect(`/student/courses/${slug}?error=payment_required`)');
content = content.replace(/href=\{\`\/student\/courses\/\$\{id\}\`\}/g, 'href={`/student/courses/${slug}`}');

fs.writeFileSync(path, content);
console.log('Fixed Materials Page');
