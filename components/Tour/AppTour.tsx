...
export function getSteps(role: string | undefined, ctx: TourData | undefined): Step[] {
  ...
  const commonStart: Step[] = [
    {
      ...
      content: (
        <div className="space-y-2 text-left">
          <p className="text-aerojet-blue text-lg font-bold">Welcome to Aerojet Academy!</p>
          <p className="text-sm text-slate-600">
            Let&apos;s take a quick tour of your portal. Each step highlights a feature and the{' '}
            <strong>key information</strong> it carries — not just what it does.
          </p>
        </div>
      ),
      placement: 'center',
    ...
    },
    {
      ...
      content: (
        <div className="space-y-1 text-left">
          ...
          <p className="text-sm text-slate-600">
            Click this info icon anytime to restart the tour. It&apos;s always available in your
            topbar.
          </p>
        </div>
      ),
    ...
    },
    {
      ...
      content: (
        <div className="space-y-1 text-left">
          ...
          <p className="text-sm text-slate-600">
            This banner carries personalized messages and critical academy announcements. Check it
            for payment deadlines, exam changes, and account alerts.
          </p>
        </div>
      ),
      placement: 'bottom',
    ...
    },
    {
      ...
      content: (
        <div className="space-y-1 text-left">
          ...
          <p className="text-sm text-slate-600">
            Access your profile, switch between light and dark themes, or sign out. You can restart
            this tour anytime from here.
          </p>
        </div>
      ),
      placement: 'top',
  ...
  const commonFinish = (headline: string, sub: string): Step => ({
    ...
    content: (
      <div className="space-y-2 text-left">
        <p className="text-aerojet-blue text-lg font-bold">{headline}</p>
        <p className="text-sm whitespace-pre-line text-slate-600">{sub}</p>
      </div>
    ),
    placement: 'center',
  })
  ...
  if (role === 'STUDENT') {
    const walletContent = (
      <div className="space-y-1 text-left">
        ...
        <p className="text-sm text-slate-600">
          All exam fees, course fees, and booking deposits are paid from here. Top up before payment
          deadlines or your exam seat reservations may be released.
        </p>
        {typeof balance === 'number' ? (
          <p className={`text-sm font-bold ${balance < 50 ? 'text-red-600' : 'text-emerald-700'}`}>
      ...
      stepWithScroll({
        ...
        content: (
          <div className="space-y-1 text-left">
            ...
            <p className="text-sm text-slate-600">
              Your study overview — deadlines, wallet status, and quick links to what needs doing
              today.
            </p>
          </div>
        ),
      ...
      stepWithScroll({
        ...
        content: (
          <div className="space-y-1 text-left">
            ...
            <p className="text-sm text-slate-600">
              Enroll in modules, access study resources, and check your academic calendar for term
              dates and deadlines.
            </p>
          </div>
        ),
      ...
      stepWithScroll({
        ...
        content: (
          <div className="space-y-1 text-left">
            ...
            <p className="text-sm text-slate-600">
              Book exam seats, join pools for shared pricing, view your exam history, and check
              results. Seat reservations expire if not confirmed before the booking deadline.
            </p>
          </div>
        ),
        placement: 'right',
      ...
      stepWithScroll({
        ...
        content: (
          <div className="space-y-1 text-left">
            ...
            <p className="text-sm text-slate-600">
              Grades, transcript, certificates, and license progress. Results are released per exam
              schedule — check back after the published date.
            </p>
          </div>
        ),
        placement: 'right',
      ...
      stepWithScroll({
        ...
        content: (
          <div className="space-y-1 text-left">
            ...
            <p className="text-sm text-slate-600">
              Classmates, attendance, seating plans, and your OJT logbook. Attendance below the
              required threshold risks enrollment status.
            </p>
          </div>
        ),
        placement: 'right',
      ...
      stepWithScroll({
        ...
        content: (
          <div className="space-y-1 text-left">
            ...
            <p className="text-sm text-slate-600">
              Payment confirmations, exam reminders, result releases, and academy announcements.
              Action-required items are flagged.
            </p>
            {unread > 0 ? (
      ...
      stepWithScroll({
        ...
        content: (
          <div className="space-y-1 text-left">
            ...
            <p className="text-sm text-slate-600">
              Live alerts surface at a glance — payments awaiting reconciliation, overdue GDPR
              requests, fraud-flagged referrals, and expiring exam bundles.
            </p>
          </div>
        ),
        placement: 'right',
      ...
      stepWithScroll({
        ...
        content: (
          <div className="space-y-1 text-left">
            ...
            <p className="text-sm text-slate-600">
              Students, instructors, examiners, applicants, and withdrawal requests. Pending
              applicant reviews and payment proofs trigger live dashboard alerts.
            </p>
            {unread > 0 ? (
              <p className="text-sm font-bold text-amber-600">{unread} unread notifications</p>
            ) : null}
          </div>
        ),
        placement: 'right',
      ...
      stepWithScroll({
        ...
        content: (
          <div className="space-y-1 text-left">
            ...
            <p className="text-sm text-slate-600">
              Payments, reconciliation, refunds, and financial reports. Payment Proofs pending more
              than 7 days risk breaching the SLA.
            </p>
          </div>
        ),
        placement: 'right',
      ...
      stepWithScroll({
        ...
        content: (
          <div className="space-y-1 text-left">
            ...
            <p className="text-sm text-slate-600">
              Exam management, pool members, seat assignments, and batch processing.
              Over/under-booked pools are flagged here.
            </p>
          </div>
        ),
        placement: 'right',
      ...
      stepWithScroll({
        ...
        content: (
          <div className="space-y-1 text-left">
            ...
            <p className="text-sm text-slate-600">
              GDPR requests and RBAC permissions. Overdue DSRs breach the 30-day Article 12(3)
              response window.
            </p>
          </div>
        ),
      ...
      stepWithScroll({
        ...
        content: (
          <div className="space-y-1 text-left">
            ...
            <p className="text-sm text-slate-600">
              The bell surfaces pending actions across payments, applicants, and enrollments. Check
              the dashboard AlertsCenter for full detail.
            </p>
          </div>
        ),
        placement: 'bottom',
      ...
      stepWithScroll({
        ...
        content: (
          <div className="space-y-1 text-left">
            <p className="font-bold">Dashboard</p>
            <p className="text-sm text-slate-600">
              Your teaching overview — upcoming classes, schedule, and grading workload.
            </p>
          </div>
        ),
        placement: 'right',
      ...
      stepWithScroll({
        ...
        content: (
          <div className="space-y-1 text-left">
            ...
            <p className="text-sm text-slate-600">
              Your teaching schedule and availability preferences. Keep availability current so the
              scheduler can assign you to classes.
            </p>
          </div>
        ),
        placement: 'right',
      ...
      stepWithScroll({
        ...
        content: (
          <div className="space-y-1 text-left">
            ...
            <p className="text-sm text-slate-600">
              Submissions awaiting your review. Return grades within the published turnaround window
              to keep results on schedule.
            </p>
            {pendingGrading > 0 ? (
              <p className="text-sm font-bold text-amber-600">
      ...
      stepWithScroll({
        ...
        content: (
          <div className="space-y-1 text-left">
            ...
            <p className="text-sm text-slate-600">
              Enter and submit candidate results. Results are locked once published — review before
              submitting.
            </p>
          </div>
        ),
      ...
      stepWithScroll({
        ...
        content: (
          <div className="space-y-1 text-left">
            ...
            <p className="text-sm text-slate-600">
              Audit trail and compliance flags for exam administration. Resolve flagged items before
      ...
      stepWithScroll({
        ...
        content: (
          <div className="space-y-1 text-left">
            ...
            <p className="text-sm text-slate-600">
              Set your availability windows so the scheduler can assign you to exam sittings. Keep
              these current to avoid conflicts.
            </p>
          </div>
        ),
        placement: 'right',
  ...
  // ── APPLICANT / DEFAULT ─────────────────────────────────────────────────
  const stageLabel = appStage
    ? `Current stage: ${appStage}`
    : 'Check the sidebar for your current step.'
  return [
    ...commonStart,
    stepWithScroll({
      target: '[data-tour-id="nav-dashboard"]',
    ...
    stepWithScroll({
      ...
      content: (
        <div className="space-y-1 text-left">
          ...
          <p className="text-sm text-slate-600">
            Track your pipeline: documents, aptitude test, interview, and medical. Each stage must
            be complete before the next unlocks.
          </p>
        </div>
      ),
      placement: 'right',
    ...
    stepWithScroll({
      ...
      content: (
        <div className="space-y-1 text-left">
          ...
          <p className="text-sm text-slate-600">
            Upload required documents. Incomplete documents delay your application — flagged here
            with expiry dates.
          </p>
        </div>
      ),
    ...
    stepWithScroll({
      ...
      content: (
        <div className="space-y-1 text-left">
          <p className="font-bold">Wallet</p>
          <p className="text-sm text-slate-600">
            Pay your registration fee and other dues. An unpaid balance blocks course enrollment and
            exam booking.
          </p>
          {typeof balance === 'number' ? (
    ...
    stepWithScroll({
      ...
      content: (
        <div className="space-y-1 text-left">
          ...
          {unread > 0 ? (
            <p className="text-sm font-bold text-amber-600">{unread} unread — action required</p>
          ) : null}
        </div>
      ),
      placement: 'right',
