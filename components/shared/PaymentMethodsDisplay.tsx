...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button onClick={handleCopy} className="rounded-md p-1 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600" title="Copy">
      {copied ? <Check className="h-3.5 w-3.5 text-green-500" /> : <Copy className="h-3.5 w-3.5" />}
    </button>
  )
}

function DetailRow({ label, value, mono, copyable }: { label: string; value: string | null; mono?: boolean; copyable?: boolean }) {
  if (!value) return null
  return (
    <div className="flex items-start justify-between gap-4 border-b border-slate-50 pb-2 last:border-0 dark:border-white/5">
      <dt className="text-[10px] font-black tracking-widest text-slate-400 uppercase">{label}</dt>
      <dd className={`flex items-center gap-1.5 text-right text-sm font-bold text-slate-800 dark:text-slate-200 ${mono ? 'font-mono' : ''}`}>
        {value}
...
function BankTransferDetails({ method, reference, referenceLabel }: { method: PaymentMethodData; reference?: string; referenceLabel?: string }) {
  return (
...
function MobileMoneyDetails({ method, reference, referenceLabel }: { method: PaymentMethodData; reference?: string; referenceLabel?: string }) {
  ...
  return (
    <dl className="space-y-3">
      <DetailRow label="Provider" value={providerNames[method.momoProvider || ''] || method.momoProvider} />
      <DetailRow label="Phone Number" value={method.momoNumber} mono copyable />
      {method.momoMerchantCode && <DetailRow label="Merchant Code" value={method.momoMerchantCode} mono copyable />}
...
export default function PaymentMethodsDisplay({ methods, reference, referenceLabel }: PaymentMethodsDisplayProps) {
  ...
  if (methods.length === 0) {
    return (
      <div className="rounded-xl border border-slate-200 bg-slate-50 p-6 text-center dark:border-white/5 dark:bg-slate-900">
        <p className="text-sm text-slate-500">No payment methods available. Please contact the academy.</p>
      </div>
    )
  }
    ...
    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-white/10 dark:bg-slate-900">
      ...
      <div className="p-5">
        {activeMethod.type === 'BANK_TRANSFER' && (
          <BankTransferDetails method={activeMethod} reference={reference} referenceLabel={referenceLabel} />
        )}
        {activeMethod.type === 'MOBILE_MONEY' && (
          <MobileMoneyDetails method={activeMethod} reference={reference} referenceLabel={referenceLabel} />
        )}
